...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_default1_heavy-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_default1_heavy_bundle.jsonl"
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        if candidate.bucket == "binary_prompt_local_exact":
            variant_styles = ["exact_rule_commit"]
        else:
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_default1_heavy",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core structured-heavy exact-binary overlay, a heavier anti-default1 counterexample lane on hard binary IDs, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-default1-heavy corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, shifting extra overlay budget into structured-byte exact closure and a heavier anti-default1 lane, "
            "and retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_default1_heavy_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-default1-heavy report",
        "",
        "## Purpose",
        "",
        ...
        "keeps the logic / permutation / prompt-local lanes narrower than v6-core,",
        "adds a heavier anti-default1 counterexample lane on measured hard binary IDs,",
