...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_common_only_boost-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_common_only_boost_bundle.jsonl"
...
NUMERIC_GUESS_COMMON_POOL_MIN = 4
NUMERIC_GUESS_COMMON_BONUS_REPEAT = 1
NUMERIC_GUESS_VERY_COMMON_BONUS_REPEAT = 2
NUMERIC_GUESS_VERY_COMMON_POOL_MIN = 10
...
def select_candidates(candidates: list[Candidate], bucket_limits: dict[str, int]) -> list[Candidate]:
    ...
    common_operator_order = [
        operator for operator in operator_order if len(numeric_guess_by_operator[operator]) >= NUMERIC_GUESS_COMMON_POOL_MIN
    ]
    numeric_prefill_limit = min(bucket_limits[numeric_bucket], len(common_operator_order))
    for operator in common_operator_order:
    ...
    for candidate in numeric_candidates:
        if candidate.row["category"] != "equation_numeric_guess":
            continue
        operator = candidate.row.get("symbol_query_operator", "")
        if len(numeric_guess_by_operator.get(operator, [])) < NUMERIC_GUESS_COMMON_POOL_MIN:
            continue
...
def build_overlay_rows(selected: list[Candidate], repeat_counts: dict[str, int]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate in selected:
        ...
        if candidate.bucket == "surface_numeric_answer_only" and candidate.row["category"] == "equation_numeric_guess":
            ...
            if operator_pool_count >= NUMERIC_GUESS_VERY_COMMON_POOL_MIN:
                base_variant_count += NUMERIC_GUESS_VERY_COMMON_BONUS_REPEAT
            elif operator_pool_count >= NUMERIC_GUESS_COMMON_POOL_MIN:
                base_variant_count += NUMERIC_GUESS_COMMON_BONUS_REPEAT
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_common_only_boost",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "a numeric answer-only support lane with one-per-operator coverage across equation_numeric_guess rows, and a heavier anti-default1 pack, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-numeric-guess-common-only-boost corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, reallocating more budget into prompt-local exact rows, "
            "adding both short-closure and token-skill lanes, adding a numeric answer-only support lane with one-per-operator coverage across only the common equation_numeric_guess operators plus extra common-pool repetition, "
            "and strengthening the anti-default1 lane while retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_numeric_guess_common_only_boost_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-numeric-guess-common-only-boost report",
        "",
        "## Purpose",
        "",
        ...
        "adds a numeric answer-only support lane for equation_numeric rows with coverage only across common guess operators and extra common-pool repetition,",
