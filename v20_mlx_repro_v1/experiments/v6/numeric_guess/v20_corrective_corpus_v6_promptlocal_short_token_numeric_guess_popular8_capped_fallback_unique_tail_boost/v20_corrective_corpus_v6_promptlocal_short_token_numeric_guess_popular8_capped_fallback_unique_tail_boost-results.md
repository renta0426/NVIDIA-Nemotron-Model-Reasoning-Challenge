...
- Pause note (2026-04-20): その後も `latest_train_report.json` は出ず、`top` では `0% CPU`, 約 `88 GB RSS`, `state=stuck` のまま mainline 2 本を圧迫したため、この live train は一旦停止した。queue / eval watcher は残してあり、resource 条件が戻ったら再開できる
