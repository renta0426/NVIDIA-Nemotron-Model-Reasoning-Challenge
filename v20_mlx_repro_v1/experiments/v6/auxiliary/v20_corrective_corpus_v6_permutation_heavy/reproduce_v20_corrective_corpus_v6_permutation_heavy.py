...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_permutation_heavy-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_permutation_heavy_bundle.jsonl"
...
V6_STRUCTURED_FAMILY_QUOTAS = {
    "xor(shl,shr)": 28,
    "choose(shl,shr,rol)": 16,
    "choose(shl,shr,ror)": 16,
    "majority(ror,shl,shr)": 14,
    "majority(rol,shl,shr)": 14,
    "xor(ror,shl)": 10,
    "or(rol,shr)": 8,
    "or(ror,shr)": 8,
}

STRUCTURED_SPILLOVER_SLOTS = 18
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_permutation_heavy",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core permutation-heavy exact-binary overlay, anti-default1 reinforcement on hard binary IDs, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 permutation-heavy corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, shifting extra overlay budget into permutation and bijection exactness, "
            "and retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_permutation_heavy_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 permutation-heavy report",
        "",
        "## Purpose",
        "",
        "This bundle keeps the v20 base snapshot,",
        "adds a permutation-heavy exact binary teacher mix with more bijection and permutation budget,",
        "keeps the structured-byte lane narrower than v6-core while preserving the logic lane,",
