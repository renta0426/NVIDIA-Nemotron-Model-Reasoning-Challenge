...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_fresh_local300_bit_miss_heavy_no_crypt-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_fresh_local300_bit_miss_heavy_no_crypt_bundle.jsonl"
...
DEFAULT_BUCKET_LIMITS = {
    ...
    "surface_binary_prompt_local_answer_only": 112,
    "surface_binary_structured_answer_only": 112,
    "surface_numeric_answer_only": 64,
    "surface_cryptarithm_answer_only": 0,
...
# The 16 exact local300 bit misses from v20_mlx_repro_v1_fullrun_targetfix_mb1_nobc (254/300 = 0.8467).
# NOTE: 0ec17d2e is absent from train_recommended_learning_target_v1.csv; it is included here so
# it appears in diagnostics, but build_budgeted_candidates will gracefully skip it at selection time.
BIT_LOCAL300_MISS_IDS = {
    "000b53cf",
    "012fb81b",
    "01e09228",
    "02a66bcb",
...
BIT_LOCAL300_MISS_SUPPORT_BONUS_REPEAT = 1
...
BIT_FAMILY_REBALANCE_STRUCTURED_TARGET_COUNTS = {
    "choose(shl,shr,rol)": 30,
    "majority(rol,shl,shr)": 28,
    "majority(ror,shl,shr)": 28,
    "choose(shl,shr,ror)": 24,
    "choose(shl,shr,nibble_swap)": 16,
    "choose(rol,ror,shr)": 12,
    "or(rol,shl)": 8,
    "or(rol,shr)": 6,
    "xor(ror,shl)": 4,
    "or(ror,shr)": 2,
...
# Cryptarithm miss support removed in this branch (budget reallocated to binary surface lanes).
CRYPT_V4_PARTIAL_MISS_IDS: set[str] = set()
CRYPT_V4_PARTIAL_MISS_SUPPORT_BONUS_REPEAT = 0
# The 2 exact local300 equation_numeric_guess misses.
NUMERIC_LOCAL300_MISS_IDS = {
    "065f9dea",
    "0c0683c3",
}
...
# 0184a864 is also in MEASURED_MANDATORY_IDS["surface_cipher_boxed"] and is a local300 cipher miss.
CIPHER_V4_PARTIAL_MISS_IDS = {"0184a864", "018c6f61", "13db9692", "16642d10"}
...
HARD_DEFAULT1_TARGET_IDS = {
    ...
    "59c78e51",
    "a6192d29",
    # Local300 bit misses not previously in this set (verified_trace_ready exact binary lanes).
    "02a66bcb",
    "034fb629",
    "05ca617c",
    "07e8cf66",
    ...
    if candidate.bucket.startswith("binary_"):
        ...
        if bonus_variant_count:
            ...
            for index in range(bonus_variant_count):
                ...
                rows.append(boosted)
        return rows
    if candidate.row["id"] in BIT_LOCAL300_MISS_IDS and candidate.bucket.startswith("surface_binary_"):
        base_variant_count += BIT_LOCAL300_MISS_SUPPORT_BONUS_REPEAT
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    for row_id in sorted(BIT_LOCAL300_MISS_IDS):
    ...
    for row in train_recommended_rows:
        ...
        if row_id in NUMERIC_LOCAL300_MISS_IDS:
            source_tags.add("local300_numeric_miss_support")
    ...
    diagnostics = {
        ...
        "local300_bit_miss_ids": sorted(BIT_LOCAL300_MISS_IDS),
        ...
        "local300_numeric_miss_ids": sorted(NUMERIC_LOCAL300_MISS_IDS),
...
def select_candidates(candidates: list[Candidate], bucket_limits: dict[str, int]) -> list[Candidate]:
    ...
    for candidate in numeric_candidates:
        if candidate.row["id"] not in NUMERIC_LOCAL300_MISS_IDS:
            continue
    ...
    for candidate in candidates:
        if candidate.row["id"] not in BIT_LOCAL300_MISS_IDS:
            continue
        if candidate.bucket not in {
            "surface_binary_prompt_local_answer_only",
            "surface_binary_structured_answer_only",
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_fresh_local300_bit_miss_heavy_no_crypt",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            "Single-file training bundle. "
            "Includes the v20 base snapshot minus metric-wrong base rows, "
            "plus a promptlocal-heavy exact-binary overlay with short-closure, token-skill, and heavier anti-default1 pack, "
            "with binary surface lanes boosted to cover all 16 local300 bit misses, "
            "a plain popular8 numeric support lane prioritising the 2 local300 equation_numeric_guess misses, "
            "and cipher surface support for the local300 miss 0184a864. "
            "Cryptarithm support removed to free budget for bit miss focus."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 fresh-local300-bit-miss-heavy-no-crypt corrective corpus for the v20 MLX local research path. "
            "Heavily emphasises support for the 16 local300 bit misses and the 2 local300 equation_numeric_guess misses. "
            "Adds cipher surface support for the local300 cipher miss 0184a864. "
            "Cryptarithm support removed; freed budget reallocated to binary surface miss lanes. "
            "Boosts binary family rebalance for choose(shl,shr,rol), majority(rol/ror,shl,shr), choose(shl,shr,nibble_swap), choose(rol,ror,shr) families. "
            "Retains short-closure, token-skill, anti-default1, and minimal boxed-surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_fresh_local300_bit_miss_heavy_no_crypt_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 fresh-local300-bit-miss-heavy-no-crypt report",
        "",
        "## Purpose",
        "",
        "Fresh-restart corpus strategy targeting local score >= 0.9 from baseline 254/300 = 0.8467.",
        ...
        "adds boosted binary surface miss lanes covering all 16 local300 bit misses (incl. 0ec17d2e graceful skip),",
        "prioritises the 2 local300 equation_numeric_guess misses (065f9dea, 0c0683c3),",
        "adds cipher surface support for the local300 miss 0184a864,",
        "applies heavier anti-default1 supervision on all verified local300 bit miss IDs,",
        "boosts family rebalance for choose(shl,shr,rol), majority(rol/ror,shl,shr), choose(shl,shr,nibble_swap), choose(rol,ror,shr),",
        "and removes cryptarithm support entirely to free budget for bit miss focus.",
        "",
