...
VERSION_NAME = "v20_mlx_v4_mainline_submission_repro"
VERSION_DIR = Path(__file__).resolve().parent
RESULTS_MD_PATH = VERSION_DIR / f"{VERSION_NAME}-results.md"
SOURCE_RUN_ROOT = REPO_ROOT / "v20_mlx_repro_v1/outputs/v20_mlx_v4_mainline_mb1_nobc"
DEFAULT_SOURCE_ADAPTER_DIR = (
    SOURCE_RUN_ROOT / "adapter"
)
DEFAULT_REFERENCE_MODEL_ROOT = (
    SOURCE_RUN_ROOT / "shadow_model"
)
DEFAULT_OUTPUT_ROOT = (
    VERSION_DIR / "outputs" / VERSION_NAME
)
...
SOURCE_VALIDATION_SCORE = "813/950 = 0.8558"
SOURCE_PROXY_SCORE = "179/200 = 0.8950"
SOURCE_PUBLIC_SCORE = "0.85 x3, 0.86 x2"
...
def write_results_md(manifest: dict[str, Any]) -> None:
    zip_members = manifest["submission_zip_members"]
    lines = [
        ...
        f"- Source run: `v20_mlx_v4_mainline_mb1_nobc`",
        f"- Source adapter dir: `{manifest['source_adapter_dir']}`",
        f"- Reference model root: `{manifest['reference_model_root']}`",
        ...
        "## Source scores",
        "",
        f"- validation: `{SOURCE_VALIDATION_SCORE}`",
        f"- proxy: `{SOURCE_PROXY_SCORE}`",
        f"- public leaderboard: `{SOURCE_PUBLIC_SCORE}`",
        "",
        "## Export summary",
        "",
    ...
    lines.extend(f"- `{member}`" for member in zip_members)
    lines.extend(
        [
            "",
            "## Interpretation",
