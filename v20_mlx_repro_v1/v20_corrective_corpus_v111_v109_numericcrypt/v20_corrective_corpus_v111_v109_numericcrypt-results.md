# v20_corrective_corpus_v111_v109_numericcrypt

- created_at: 2026-04-30T13:41:23.981147+00:00
...
- analysis basis: `README.md` highlights especially weak base slices in `Equation Numeric (Guess)`, `Cryptarithm (Deduce)`, `Cryptarithm (Guess)`, `Bit Manipulation`, and `Cipher`. This branch keeps the v109 post-bit deduce frontier and adds one more numeric replay pass.
- local target: current best local300 `0.846667` -> aim for `> 0.9` by preserving the broader v109 recovery stack while re-pressuring the README `Equation Numeric (Guess)` weakness.
- status: bundle generated; model score not yet measured.
- planned run name: `v20_mlx_v111_v109_numericcrypt_mlxdir_mb1_nobc_ckpt20`
...
- Retain the broader v12 bit-binary mainline core and the v109 stack that already refreshed exact-safe BIT plus focused crypt deduce pressure.
- Add another numeric operator plus quote replay so the branch revisits the README `Equation Numeric (Guess)` slice after the post-bit crypt refresh.
...
- path: A-Open-ProgressPrizePublication/nemotron/training/sft/MLX/v20_corrective_corpus_v111_v109_numericcrypt_bundle.jsonl
- base_examples: 7830
- overlay_examples: 21799
- total_examples: 29629
- total_steps: 927
- total_tokens: 33898256
- max_seq_len: 7971
