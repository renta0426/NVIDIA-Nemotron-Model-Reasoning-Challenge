...
- By **step 160**, the run was still healthy (`train_loss = 0.0005083884614482659`, `lr = 7.020408163265306e-05`, peak memory `222.0017 GB`) and the comparison had flattened almost perfectly. Over steps `129-160`, nobc beat baseline on `16` steps and trailed it on `16`, making this window the clearest “near tie” segment so far.
- The best nobc advantage in that `129-160` window was at **step 138**: baseline `0.0016220388534255526` vs nobc `0.0008923456828183532` (delta `-0.0007296931706071994`). The largest regression in the same window was at **step 147**: baseline `0.0004616750649931808` vs nobc `0.0016871641286571475` (delta `+0.0012254890636639666`).
- The latest `step 160` comparison itself was almost unchanged but still slightly favorable to nobc: baseline `0.0005202724311258987` vs nobc `0.0005083884614482659` (delta `-0.000011883969677632753`). The last eight deltas leading into that point were `153:-0.00027162452686282603`, `154:+0.00012207425001827113`, `155:+0.000219925909688761`, `156:-0.0005802946170537093`, `157:-0.000018109918456790208`, `158:-0.00028127856317751627`, `159:-0.00036111277941505574`, `160:-0.000011883969677632753`.
- Interpretation at the step160 milestone: by the middle of the run, the no-bias-correction path still has not separated itself decisively from the baseline MLX Adam path. The live evidence now reads as **“sometimes clearly better, sometimes clearly worse, and sometimes almost tied”**, which keeps the question squarely on the final trained adapter quality rather than the intermediate loss curve.
- The full no-bias-correction retrain then **completed successfully** at step `245/245` with `train_loss = 0.00031850469393745887`, `trained_tokens = 26568807`, `peak_memory_gb = 222.0017`, and `total_elapsed_seconds = 74801.3432`. Relative to the completed baseline MLX run, that is a slightly **higher** final train loss (`+9.732627311848335e-06`), slightly **higher** peak memory (`+0.0608 GB`), and slightly **faster** wall time (`-445.7722 s`).
- The automatic 8-row smoke validation finished normally but came in **below** the prior gate: **`6 / 8 = 0.75`** on `adapter_validation_stratified_category_8_of_950`, versus the earlier baseline smoke **`7 / 8 = 0.875`**. Category-wise the misses were `bit_manipulation 1 / 2` and `cryptarithm_deduce 0 / 1`, with the other sampled families (`cipher`, `gravity`, `numeral`, `unit_conversion`) still correct.
- Because that smoke run was a clean evaluation rather than a crash or malformed-output failure, its artifacts were archived to `adapter_validation_smoke8_snapshot/` and the 300-row stratified adapter-validation was **manually launched anyway** to obtain the final README-aligned local score. The automatic `eval300` chain watcher was originally gated on smoke `>= 7/8`, so this manual launch is an intentional override to finish the experiment rather than a change to the underlying pass/fail heuristic.
- The manual eval300 launch uses the same MLX validation settings as the successful baseline local300 run: `validation_subset_size=300`, `validation_subset_mode=stratified-category-proportional`, `eval_shards=4`, `max_num_seqs=4`, `prompt_chunk_size=4`, `prefill_batch_size=4`, and `completion_batch_size=4`. Four detached shard jobs were started together, and a detached merge watcher now waits for all shard `validation_progress.json` files to reach `status=complete` before running `merge-adapter-validation` and `postprocess-run --postprocess-eval-kind adapter-validation`.
- Early manual eval300 progress after that launch remained healthy and did **not** show an obvious slowdown versus the baseline local300 run. By the first tracked mid-point, the nobc eval had reached **`100 / 300`** completed rows with shard-local progress `25 / 75`, `24 / 75`, `28 / 75`, and `23 / 75`, all still `running`.
