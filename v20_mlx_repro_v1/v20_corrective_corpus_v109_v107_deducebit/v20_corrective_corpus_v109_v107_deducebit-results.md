# v20_corrective_corpus_v109_v107_deducebit

- created_at: 2026-04-30T13:34:23.599075+00:00
...
- analysis basis: `README.md` highlights especially weak base slices in `Cryptarithm (Deduce)`, `Cryptarithm (Guess)`, `Bit Manipulation`, `Equation Numeric (Guess)`, and `Cipher`. This branch keeps the v107 BIT-refreshed frontier and adds one more focused cryptarithm-deduce replay pass.
- local target: current best local300 `0.846667` -> aim for `> 0.9` by preserving the broader v107 recovery stack while re-pressuring the README `Cryptarithm (Deduce)` weakness.
- status: bundle generated; model score not yet measured.
- planned run name: `v20_mlx_v109_v107_deducebit_mlxdir_mb1_nobc_ckpt20`
...
- Retain the broader v12 bit-binary mainline core and the v107 stack that already refreshed numeric, cipher, and exact-safe BIT pressure.
- Add another focused deduce-only glyph_len5 replay so the branch revisits the README `Cryptarithm (Deduce)` slice after the BIT refresh.
...
- path: A-Open-ProgressPrizePublication/nemotron/training/sft/MLX/v20_corrective_corpus_v109_v107_deducebit_bundle.jsonl
- base_examples: 7830
- overlay_examples: 21533
- total_examples: 29363
- total_steps: 918
- total_tokens: 33854736
- max_seq_len: 7971
