...
必要に応じて、`revision` は **固定 commit** を入れてください。未固定なら `main` でも動きますが、本番提出用には非推奨です。

```bash
uv run python train_lora_macos.py \
...
```bash
uv run python smoke_test_adapter.py \
...
```bash
uv run python validate_submission.py --adapter_dir ./output/nemotron_lora
```
...
```bash
uv run python make_submission_zip.py \
  --adapter_dir ./output/nemotron_lora \
