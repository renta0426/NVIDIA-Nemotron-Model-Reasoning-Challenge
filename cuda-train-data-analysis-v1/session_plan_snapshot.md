...
- 次ステップは、structured byte formula の残る `7 manual + 4 exclude` を contaminated family / singleton family / ambiguous multi-pred に割って詰めるか、current `symbol` round2 core `282` 行のうち `:` / custom-op / operator-embedded tail をさらに切ること。glyph 46 行は、新しい family 仮説が出るまで hold。
