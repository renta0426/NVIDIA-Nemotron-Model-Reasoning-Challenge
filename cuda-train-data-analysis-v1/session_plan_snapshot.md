...
- 次ステップは、structured byte formula の residual `20 manual + 4 exclude` を `19 singleton / 1 ambiguous multi-pred` に割って詰めるか、current `symbol` round2 core `284` 行のうち custom-op / operator-embedded tail をさらに切ること。glyph 46 行は、新しい family 仮説が出るまで hold。
