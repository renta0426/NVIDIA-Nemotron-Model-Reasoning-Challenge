...
def collect_symbol_thin_support2_matches(prompt: str, query_text: str, answer: str) -> dict[str, Any]:
    query_match = SYMBOL_NUMERIC_EXPRESSION_PATTERN.match(str(query_text))
    if query_match is None:
        return {
            "query_operator": "",
    ...
    if query_operator == "!":
        abs_diff_exact = all(
            render_symbol_numeric_string_template(
                SYMBOL_NUMERIC_STRING_TEMPLATES["abs_diff_2d"],
                left_value_text,
                query_operator,
                right_value_text,
            ) == str(output_text)
        ...
        if abs_diff_exact and query_x_text[0] < query_y_text[0] and query_x_text[1] < query_y_text[1]:
            winning_specs.append(
                (
                    "exclamation_abs_diff_2d_both_lt",
                    "string_template",
                    render_symbol_numeric_string_template(
                        SYMBOL_NUMERIC_STRING_TEMPLATES["abs_diff_2d"],
                        query_x_text,
        ...
        quote_prefix_if_negative_exact = all(
            SYMBOL_NUMERIC_FORMATS["prefix_if_negative"](
                query_operator,
                SYMBOL_NUMERIC_FORMULAS["x_minus_y"](int(left_value_text), int(right_value_text)),
            ) == str(output_text)
            for left_value_text, right_value_text, output_text in same_operator_examples
        )
        diff_value = abs(int(query_x_text) - int(query_y_text))
        if quote_prefix_if_negative_exact and int(query_x_text) > int(query_y_text) and diff_value >= 40:
            winning_specs.append(
                (
                    "quote_prefix_if_negative_large_positive",
                    "string_template",
                    SYMBOL_NUMERIC_FORMATS["prefix_if_negative"](
                        query_operator,
                        SYMBOL_NUMERIC_FORMULAS["x_minus_y"](int(query_x_text), int(query_y_text)),
                    ),
                )
            )
    return {
...
def apply_symbol_thin_support2_promotions(analysis_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    analysis_df = analysis_df.copy()
    numeric_mask = (
        (analysis_df["family"] == "symbol_equation")
        & (analysis_df["template_subtype"] == "numeric_2x2")
    ...
    row_match_map: dict[str, dict[str, Any]] = {}
    for row in analysis_df.loc[numeric_mask, ["id", "prompt", "query_raw", "answer", "selection_tier"]].to_dict(orient="records"):
        match_info = collect_symbol_thin_support2_matches(
            str(row["prompt"]),
            str(row["query_raw"]),
            str(row["answer"]),
        )
    ...
    if promote_predictions:
        ...
        analysis_df.loc[promote_mask, "audit_priority_score"] = 0.0
        analysis_df.loc[promote_mask, "audit_reasons"] = ""
        analysis_df.loc[promote_mask, "analysis_notes"] = "symbol_thin_support2_subfamily"
        analysis_df.loc[promote_mask, "suspect_label"] = False
