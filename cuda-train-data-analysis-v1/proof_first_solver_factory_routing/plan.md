...
public eval では `glyph_len5` が `0/20` で、symbol family 崩壊の主因になっている。train 側にも `glyph_len5` は `823` 行あり、完全放置のコストは大きい。
...
7. symbol numeric accuracy
8. symbol glyph accuracy
9. gravity / roman / text / unit accuracy
...
6. manual-origin answer-only rescue memo
7. family-balanced route-aware training mix memo
