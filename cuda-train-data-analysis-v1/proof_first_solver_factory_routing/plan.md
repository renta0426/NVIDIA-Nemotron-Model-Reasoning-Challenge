...
## 2.5 v3f 実験から今回追加で固定すること

README.md 前提の Accuracy 契約と、今回の v3f 実験の実測を合わせると、ここで新たに固定すべき点は明確である。

1. v3f は specialized binary では改善したが、public 60-row binary では boxed 崩壊ではなく content miss 増加で落ちた
2. したがって「boxed を守れたから安全」という判定は不十分で、`format_ok_content_wrong` を主警戒指標に置く必要がある
3. proof-first routing でも、coarse `route_closure_only` を増やしすぎると同じ失敗形を再生産する危険がある
4. したがって primary lane は exact-route verified rows、coarse answer-only は低比率の補助 lane として扱う
5. binary では `binary_structured_byte_formula_abstract` の drift cleanup と `binary_structured_byte_not_formula` の明示的強化を最初から優先する

この追記は、plan の方向性を変えるためではなく、どの lane を主役にするかを v3f 実測で確定させるためのものである。
...
3. answer_only_keep は route plus closure-only あるいは query-commit の補助教師として追加するが、binary では exact-route verified を主役にした後段の低比率 lane に固定する
...
ただし binary では、v2 base に既に含まれている strongest verified rows へ route supervision が届かない場合がある。

このときは full replacement ではなく、次の条件を満たす bounded exact clone を例外として許可する。

1. 対象は structured bottleneck family に限定する
2. 元 row は残し、新しい ID を持つ route-aware exact row として追加する
3. closure-only clone は作らない
4. clone は exact lane の不足分を埋める範囲に限定する

これにより augmentation-first を維持したまま、v2 既出行にも route signal を注入できる。
...
3. coarse route でも `bit_manipulation` 一段で止めず、少なくとも `bit_structured_byte_formula`、`bit_permutation_inversion`、`bit_other` など template-subtype 水準までは落とす
...
加えて `binary_structured_byte_formula_abstract` は、safe 系強化で改善した後も abstract drift が残りやすいので、structured verified の中でも独立 bucket として QC を持つ。
...
ただし v3f の実測から、binary では boxed stability 単独では gate として弱いことも分かった。したがって binary branch では、boxed success に加えて `format_ok_content_wrong` を primary fail-fast 指標に置く。
...
### 7.2.2 binary-only delta の固定ルール

今回の single-file builder で先に回す binary-only delta では、mixed corpus 全体比率とは別に、次を固定する。

1. exact-route verified を primary lane とし、binary delta 内の `75-85%` を占めさせる
2. coarse answer-only は `15-25%` の補助 lane に固定する
3. 初回 ablation は `exact-route verified only` を先に回し、その後に bounded coarse answer-only を足す
4. coarse answer-only は family-only `bit_manipulation` に丸めず、template-subtype 水準の coarse route を残す
5. `route_closure_only` は evidence の代用品ではなく、trace 非一意 row に対する boxed-preserving support teacher としてのみ使う
6. v2 既出で blocked される structured verified rows は、新しい ID の bounded exact clone として exact quota の不足分だけ補う

この固定ルールにより、binary delta が「boxed は整うが content miss が増える」方向へ寄るのを防ぐ。
...
したがって、この plan で作る route-aware delta は v2 augmentation の形で設計しても、実装ファイル自体は `cuda-train-data-analysis-v1/proof_first_solver_factory_routing/` 配下の single-file builder として管理する。

そのうえで、最終 handoff 時に v5 側から読める CSV / manifest を渡す。
...
ここでいう structured verified binary には、v2 既出行であっても bounded exact clone として route-aware 化する slice を含める。

ただし実験順は固定する。

1. exact-route verified only
2. exact-route verified plus bounded coarse answer-only
3. exact-route verified plus bounded coarse answer-only plus abstract / not-formula focused rebalance

binary track は、この順に ablation しない案を不採用とする。
...
7. family-balanced route-aware mix manifest for later v5 handoff
