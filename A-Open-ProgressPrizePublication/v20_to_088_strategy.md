...
### 2. 根本原因：teacher solver の `default 1` fallback が誤答を学習データに混入

`A-Open-ProgressPrizePublication/nemotron/reasoning/*.txt` を調査した結果：

- binary reasoning 1602 ファイル中 **229 ファイル (14.3%)** が `default 1` を含む
- v20 学習トークンスナップショット 7830 件中 **92 件 (1.2%)** が `default 1` 問題に対応
- これらは合計 **648,074 tokens (2.33%)** を占める

決定的な事実は、**always-wrong 10 行の teacher reasoning がすべて `default 1` を含み、すべて誤答**であること：

| always-wrong ID | teacher `default 1` 数 | teacher 回答 | gold 回答 | 一致 |
| --- | --- | --- | --- | --- |
| a6192d29 | 6 | 01111110 | 00001000 | ✗ |
| 2d790c98 | 4 | 01111111 | 01111011 | ✗ |
| 12154247 | 5 | 11111101 | 10111101 | ✗ |
| ... (全 10 行同様) | | | | ✗ |

一方、**v1_only_win 3 行と corrective_fixed 3 行の teacher reasoning は `default 1` を含まず、すべて正答**：

| v1_only_win ID | teacher d1 | teacher 正否 |
| --- | --- | --- |
| 012fb81b | 0 | ✗ (別理由) |
| 0520a6ec | 0 | ✓ |
| c30a782a | 0 | ✓ |

つまり **teacher solver が `default 1` を使う問題は、学習データそのものが誤答を教えている**。model はこの「分からないときは 1 を出す」パターンを忠実に学習し、test 時にも一般化して適用する。

### 3. v1 の成功メカニズム再解釈

v1 が proxy binary を +4 した理由は「binary overlay を足したから」ではなく、**binary 正答例の比率が上がった結果、`default 1` パターンの重みが相対的に下がったから**。raw output を見ると：

- `c30a782a`: v20 は `default 1 = 1` → v1 は `AND-NOT07 = AND(0,NOT(1)) = 0` （正しい演算を実行）
- `012fb81b`: v20 は `default 1 = 1` → v1 は `AND04 = AND(1,1) = 1` （正しい演算）
- `0520a6ec`: v20 は `default 1 = 1` → v1 は `AND-NOT26 = AND(0,NOT(0)) = 0` （正しい演算）

**v1 は model から `default 1` パターンを「追い出した」**。しかし overlay が不十分で不安定だったため、v2 で戻ってしまった。

### 4. v2 の失敗メカニズム再解釈

v2 が proxy binary を -3 した理由は「guardrail が binary と干渉した」だけでなく、**binary overlay が v1 より少なく、`default 1` パターンが再浮上した**：

- `b9500f41`: v20/v1 は `I7` → v2 は `default 1` に退行
- `c30a782a`: v1 は正しい演算 → v2 は `default 1` に退行
- `8e5d6fe6`: v20/v1 は正常 → v2 は `OR-NOT50 = OR-NOT05` 無限ループ

### 5. v3 戦略に対する評価

#### 正しい点

1. **3-line 分離設計は正しい** — binary content 問題と surface/format 問題は機構が完全に異なる
2. **proxy-led promotion は正しい** — validation は v2 で micro-improvement を見せつつ proxy/public は下がった
3. **token budget 目標 (34-36M) は妥当** — v1 (39M) と v2 (32M) の間

#### 見直すべき点

1. **`binary_precision_core` の方法論が根本的に間違っている**
   - 現戦略: 「正しい binary overlay を足す」(ADDITIVE)
   - 正しいアプローチ: 「誤った `default 1` 学習例を除去する」(SUBTRACTIVE)
   - **92 件の `default 1` 含有トークンスナップショット (648K tokens) を学習データから除去**すべき
   - これは v20 全体の 1.2% / 2.3% tokens で、副作用は最小限
   - v1/v2 が overlay で間接的にやろうとしたことを、直接的に実現できる

2. **always-wrong 10 行は学習データ品質の限界であり、overlay では解決不可能**
   - teacher solver が `default 1` で誤答している問題は、同じ teacher で正しいデータを作れない
   - README の「higher solve rate will require new insights on how to solve the problems programmatically」はこの事実を指している
   - proxy ceiling は realistic に 180-182/200（always-correct 170 + unstable 12 の大半を回収）

3. **v3 の期待値を正確に見積もるべき**
   - `default 1` 除去で unstable 10 行のうち 6-8 行を安定正解にできる可能性
   - proxy 目標: 180-182/200 (0.90-0.91)
   - public LB 目標: proxy-public offset が 3-4% なら、0.87-0.88 が射程圏内
   - **0.88 public は 1 回の実験で確実に到達するとは言えないが、方向性は正しい**

### 6. 修正版 v3 実装方針

#### Line 1: `binary_signal_cleaning` (SUBTRACTIVE → 主攻)

- v20 base の 7830 token snapshots から、`default 1` を含む 92 件を除外
- 残り 7738 件のうち binary 1288 件は teacher 正答のみ
- これだけで `default 1` パターンの学習を根本的に抑制できる
- **token 削減: -648K (2.3%)**

#### Line 2: `binary_correct_replacement` (ADDITIVE → 副攻、オプション)

- 除去した 66 問題のうち一部について、`default 1` を使わない正しい trace を生成できるなら追加
- ただし teacher solver が `default 1` に fallback した問題は solver 限界なので、無理に生成しない
- **priority は d1 除去 > replacement 追加**

#### Line 3: `surface_boxed_repair` + `easy_family_preservation` (現戦略通り)

- v3 strategy document の Line 2/3 はそのまま有効
- ただし binary cleaning が主攻なので、これらは補助的
