...
| --- | ---: | ---: | ---: | ---: | ---: |
| bit_manipulation | 1602 | 604 | 35 | 947 | 16 |

参照: `cuda-train-data-analysis-v1/FINAL_SUMMARY_REPORT.md:55-70`, `cuda-train-data-analysis-v1/artifacts/family_summary_v1.csv:1-7`

ここで重要なのは、binary は 1,602 行あるのに **verified trace-ready が 604 行しかなく、947 行が manual** だという点である。  
...
- `bit_manipulation`: 604 verified, 35 answer-only, **947 manual**
...
- でも answer-only だけでは rule learning が弱い
...
- `bit_no_candidate_positions <= 1`: 117 行
- 残り 830 行は 2〜8 bit が未説明
- さらに 138 行は **8 bit 全部に candidate rule が立たない**
...
特に structured byte formula recovery では、`x`, `rol`, `ror`, `shl`, `shr`, `reverse`, `nibble_swap` などの canonical source から whole-byte formula を構成し、**311 semantic formula** を走査している。その結果 **71 safe formula** を見つけ、manual から **189 行を verified に回収**した。  
参照: `cuda-train-data-analysis-v1/reports/43_binary_structured_byte_formula_recovery.md:1-38`, `cuda-train-data-analysis-v1/reports/43_binary_structured_byte_formula_recovery.md:40-96`

それでも binary 最終状態はまだ `604 verified / 35 answer_only / 947 manual / 16 exclude`。  
つまり「単純 byte transform をもう少し増やせば終わる」段階はすでに過ぎている。
...
binary では 1,602 行中 604 行しか verified trace-ready でない。  
残りは answer-only か manual であり、特に 947 manual はそのまま trace 化すると危険である。  
参照: `cuda-train-data-analysis-v1/FINAL_SUMMARY_REPORT.md:100-120`, `cuda-train-data-analysis-v1/FINAL_SUMMARY_REPORT.md:402-419`
...
- manual binary 947 行へ一括で自然文 CoT を付ける
...
3. **1,602 行中 947 行が safe trace 化できず、学習教師として汚染しやすい**
