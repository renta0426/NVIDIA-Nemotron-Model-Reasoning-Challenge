...
### ここで誤解しやすい点
これは **binary-only の adapter をそのまま最終提出にする** 戦略ではありません。

`README.md` の評価では、**NVIDIA Nemotron-3-Nano-30B に 1 つの LoRA adapter を読み込み、全 test case をその adapter で解いた overall Accuracy** が最終スコアになります。  
つまり、binary だけ強くても text / gravity / unit / roman / symbol を落とした時点で、その adapter 単体は最終候補になりません。

ここで branch を分ける理由は、

- mixed SFT の単一 LoRA の中で binary を押すと general 側が巻き添えで壊れやすい
- しかし binary のボトルネックは他 family とかなり性質が違う
- なので **同じ重み空間で同時に最適化するより、役割分担して後で統合する方が安全**

だからです。

役割は次のように分かれます。

- `G` = final overall score の防波堤
- `B` = binary bottleneck を突破する specialist
- 最終候補 = `G + B merge` 後に guardrail を満たしたものだけ

したがって、この plan の狙いは「binary に全振りすること」ではなく、  
**binary を分離して強く改善したうえで、その gain を general を壊さずに持ち込むこと**です。
...
### これは submission 候補ではなく中間工程
Step 1 単体は、**binary specialist の終端形を整えるための readiness 工程**です。  
ここでの責務は「全 family を守ったまま overall を上げること」ではなく、
...
したがって、Step 1 の binary-only adapter を standalone で評価した時に、  
non-binary family が下がること自体は **ある程度は想定内** です。  
この段階で本当に見るべきなのは、
...
などであって、最終提出 suitability ではありません。

non-binary family への配慮は捨てているのではなく、  
**general anchor を別 branch で保持し、merge と guardrail で最後に回収する** 設計です。
...
## 共通原則
- `README.md` 上、最終提出は **全問題に対して読み込まれる単一の LoRA adapter** である
- したがって binary-only specialist は **中間成果物** であり、単独では提出候補にしない
...
### なぜ merge が落としどころになるか
`README.md` 上の提出物は **rank 制約内の単一 LoRA adapter** です。  
したがって、

- `B` 単体で binary が強い
- `G` 単体で general が安定

というだけでは不十分で、**最終的には 1 つにまとめて勝てるか** が本質です。

このとき merge の役割は、単なる平均ではありません。  
目的は **binary で得た gain を general stable 側へ安全に輸入すること** です。

なので merge candidate の採否は、

- binary strict exact が上がるか
- `General Stable Set` / `Symbol Watch Set` を守れるか
- 最終的に overall で得か

で決めるべきです。  
binary だけ上がって non-binary guardrail を壊す merge は、不採用にします。
