...
- latest observed eval checkpoint: `590 / 950` rows completed
- latest observed eval partial score: `506 / 590 = 0.857627`
- latest observed eval partial read: checkpoint CSV 再集計では `590` 行時点で `506` correct。直前の hard bit row `0f8fe647` は correct で通過しつつ、主失点は `cryptarithm_deduce 38 miss`, `bit_manipulation 19 miss`, `cryptarithm_guess 8 miss`, `equation_numeric_guess 6 miss`, `unit_conversion 5 miss` に集中している。したがって frontier stack の設計は引き続き bit / crypt / equation tail を優先してよい
