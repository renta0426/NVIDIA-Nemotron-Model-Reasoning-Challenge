...
def main() -> None:
    ...
    full_train_ds = build_sft_dataset(tr_df, tokenizer, style="direct")
    val_ds = build_sft_dataset(val_df, tokenizer, style="direct")

    general_model = load_lora_model(
        base_model_path,
        config,
    ...
    general_args = build_sft_config(
        OUTPUT_ROOT / "general",
        num_train_epochs=GENERAL_EPOCHS,
        max_length=MAX_LENGTH,
        learning_rate=LEARNING_RATE,
    ...
    for template in HARD_TEMPLATES:
        spec_df = tr_df[tr_df["template"] == template].copy().reset_index(drop=True)
        spec_val_df = val_df[val_df["template"] == template].copy().reset_index(drop=True)
        if len(spec_df) == 0:
            continue
        if len(spec_val_df) == 0:
            spec_val_df = val_df.copy().reset_index(drop=True)

        print_split_summary(f"specialist_{template}_train", spec_df)
        print_split_summary(f"specialist_{template}_valid", spec_val_df)
        spec_train_ds = build_sft_dataset(spec_df, tokenizer, style="direct")
        spec_val_ds = build_sft_dataset(spec_val_df, tokenizer, style="direct")

        specialist_model = load_lora_model(
            base_model_path,
            config,
            load_in_4bit=LOAD_IN_4BIT,
            lora_r=LORA_R,
            ...
            lora_dropout=LORA_DROPOUT,
            target_modules="all-linear",
            adapter_path=GENERAL_ADAPTER_DIR,
        )
        specialist_args = build_sft_config(
            OUTPUT_ROOT / f"specialist_{template}",
            num_train_epochs=SPECIALIST_EPOCHS,
            max_length=MAX_LENGTH,
            learning_rate=LEARNING_RATE,
            train_batch_size=TRAIN_BATCH_SIZE,
            eval_batch_size=EVAL_BATCH_SIZE,
        ...
        specialist_summaries.append(
            {
                "template": template,
                "train_rows": len(spec_df),
                "valid_rows": len(spec_val_df),
                "adapter_dir": str(specialist_dir),
            }
        )
        del specialist_model
        del specialist_trainer
        release_memory()

    if specialist_dirs:
        specialist_weight = (1.0 - GENERAL_WEIGHT) / len(specialist_dirs)
        merge_weights = [GENERAL_WEIGHT] + [specialist_weight] * len(specialist_dirs)
    else:
        merge_weights = [1.0]
    merge_dirs = [GENERAL_ADAPTER_DIR] + specialist_dirs
    merge_adapter_directories(merge_dirs, MERGED_ADAPTER_DIR, weights=merge_weights)
    adapter_cfg = package_adapter_dir(MERGED_ADAPTER_DIR, SUBMISSION_ZIP)
