...
## 15. BIT 大幅強化に向けた使用可能資産の棚卸し

### 15.1 先に結論

BIT 大幅強化に向けた raw material は、現時点で既にかなり揃っている。
不足しているのは「BIT 用データそのもの」ではなく、**persistent hard core を直接叩く完成済み mainline bundle** である。

したがって次に必要なのは、広い意味での新規発掘ではない。
必要なのは、既存資産を次の 3 層に分けて再構成することである。

1. public-safe / token-safe な backbone
2. audited binary exact pool
3. frontier 専用の新規 lane

ここで README の一次制約をもう一度確認しておく。

- winning solution は `04-08-16-14`
- 主差分源は `bit_manipulation`
- より高い solve rate には新しい programmatic insight が必要

つまり、BIT を大幅に伸ばすには **既存 corpus の比率調整ではなく、既存資産のうち何を backbone に残し、何を frontier teacher として再編するか** が本質である。

### 15.2 そのまま使える backbone / donor 資産

#### A. `04-08-16-14` base snapshot

役割:

- official winning run の base
- easy family と boxed-first extraction の強みを最も素直に持つ
- competition-only categories で構成される public-safe baseline

制約:

- auxiliary curriculum を含まない
- persistent binary frontier を解決した teacher ではない
- そのままでは BIT 大幅強化の source ではなく base に留まる

判断:

- **base backbone としては usable**
- **frontier BIT teacher としては不十分**

#### B. `v4_mainline`

役割:

- この corrective 系列で最良の public run
- `0.85-0.86` を出した public-safe donor
- easy-family / boxed terminal stability を壊さない local balance が取れている

制約:

- proxy binary は `79/92` に留まり、hard frontier を消していない
- README が主戦場とする BIT では `v6` に content gain を譲る

判断:

- **public-safe backbone donor として最重要**
- **BIT frontier 解決 source としては単独では不足**

#### C. `v6_mainline`

役割:

- この corrective 系列で最強の proxy binary donor
- proxy `180/200`, binary `80/92`
- `c30a782a`, `59c78e51` などの binary gain を実測で作った

bundle 上で既に usable な binary donor 構成も分かっている。
...
- official public は `0.83-0.85` で `v4` を超えない
- numeral boxed surface に blind spot が残る
- persistent hard binary watchlist は `11` 行残る

判断:

- **BIT donor branch として最重要**
- **public mainline そのものとしては昇格不可**

#### D. `v7-1` の token-safe donor 継承方法

役割:

- `v4_public_base` の token-safe reuse を回復した branch
- mixed mainline を作る際に、public-safe rows を text 再生成ではなく token-safe に継承すべきことを証明した

制約:

- branch 自体は public `0.84` で promote 不可
- easy-family recovery が主で、BIT frontier gain は弱い

判断:

- **teacher asset というより corpus construction asset として usable**
- 次の BIT mainline でも donor 継承方法として残すべき

### 15.3 監査済みで再利用できる binary exact pool

#### A. strict audited binary ledger

`cuda-train-data-analysis-v1` の strict audit で、`bit_manipulation` は次まで整理済みである。
...
これは「まだ BIT データが足りない」のではなく、**既に大きい strict verified pool を持っている**ことを意味する。

この ledger の基礎 artifact は次である。
...
- `cuda-train-data-analysis-v1/artifacts/train_verified_trace_ready_v1.csv`
- `cuda-train-data-analysis-v1/artifacts/train_recommended_learning_target_v1.csv`

判断:

- **v10 用の binary lane を作る際の master ledger としてそのまま usable**

#### B. exact disambiguation / reread / boolean4 の audited promotions

strict audit 後に defensible と確認された verified 増分は次である。

- `binary_exact_disambiguation_verified_v1.csv`: `+154`
- `binary_manual_exact_reverified_v1.csv`: `+62`
- `binary_four_bit_boolean_verified_v1.csv`: `+9`

意味:

- `stage1/stage2/structured/not_structured` をまたいでも一意に collapse する row は既に抽出済み
- prompt を読み直せば exact teacher 化できる row も追加抽出済み
- boolean4 系は既に verified teacher として再利用可能

判断:

- **exact closure lane の原料として即 usable**
- **新しい solver 発見ではなく、mainline へ未昇格の verified pool**

#### C. structured-byte family の recovery / curation 資産

structured-byte は既に broad family としてかなり掘れている。

- report 43: safe structured formulas `71`, new verified promotions `189`
- report 45: safe abstract families `10`, new verified promotions `29`
- report 56: residual manual を direct prompt reading で再確認し、manual exact verified / exclude を確定

対応 artifact も残っている。

- `binary_structured_byte_formula_candidates_v1.csv`
- `binary_structured_byte_formula_support_v1.csv`
- `binary_structured_byte_abstract_support_v1.csv`
- `binary_structured_byte_manual_exact_verified_v1.csv`
- `binary_structured_byte_manual_exact_excludes_v1.csv`

意味:

- structured-byte は「未知の長い tail」ではなく、かなり family 化済み
- 残差も broad family 問題ではなく narrow residual まで圧縮できている

判断:

- **structured-byte exact closure lane の raw pool として非常に強い**
- **新規発見より再編と query-specific closure 化が主課題**

#### D. prompt-local / hybrid / low-support 候補群

まだ mainline に昇格しきっていない候補群も artifacts として既に存在する。

- `binary_prompt_local_current_consensus_candidates_v1.csv`
- `binary_prompt_local_extended_candidates_v1.csv`
- `binary_prompt_local_nested_candidates_v1.csv`
- `binary_prompt_local_nested_support_v1.csv`
- `binary_prompt_local_extended_support_v1.csv`
- `binary_hybrid_consensus_candidates_v1.csv`
- `binary_affine_mismatch_candidates_v1.csv`
- `binary_structured_byte_support3_answer_only_candidates_v1.csv`
- `binary_structured_byte_low_support_answer_only_candidates_v1.csv`

意味:

- ここは verified 完成品ではないが、frontier 専用 lane を作る raw material には十分
- 特に prompt-local と hybrid は persistent hard core へ近い

判断:

- **そのまま mainline teacher にするのではなく、frontier 専用再審査 pool として usable**

### 15.4 既に実測で価値が確認された BIT 実験資産

#### A. `v1` が示した narrow binary overlay の価値

`v1` は proxy を `176/200 -> 178/200`、binary を `76/92 -> 80/92` まで押し上げた。
特に target bucket では、structured binary の gain が実測で確認されている。

これは重要で、**narrow binary corrective 自体は効く**ことが既に証明済みである。
したがって次に必要なのは「binary overlay をやるかどうか」ではなく、「どの binary asset を mainline teacher に昇格するか」である。

#### B. `v5a` が示した negative evidence

`v5a` は binary を厚くしたように見えて、実際には

- repeated binary rows `656 -> 570`
- permutation family solver rows `2 + 1`
- structured-byte / abstract 寄りに偏重

という構成だった。

この run は次の negative evidence として usable である。

- permutation / bijection を薄くすると proxy binary は落ちる
- abstract family 化だけでは hard core を消せない
- surface lane は cheap だが、それだけでは public は戻らない

判断:

- **teacher を作る asset ではなく、配分を誤らないための制約 asset**

#### C. `v8` と `v9` が示した current non-solution

`v8` は broad exact expansion を行ったが、binary proxy は `78/92` に留まり、public も `0.83-0.84` だった。
`v9` は matching を戻したが、A-Open history 上それは既知手法であり、本命ではなく diagnostic branch と判断した。

この 2 本から得られる usable lesson は明確である。

- broad exact answer expansion だけでは足りない
- auxiliary restoration だけでも足りない
- 次に必要なのは exact frontier teacher の更新である

判断:

- **v8/v9 自体は donor corpus としては弱い**
- **ただし「何を mainline にしないか」を教える負例 asset として重要**

### 15.5 参考 asset だが、そのまま mainline teacher ではないもの

#### A. A-Open bit auxiliary decomposition

`matching` は A-Open の bit reasoner から抽出された local subskill であり、token-level decomposition の設計思想自体は今も有効である。

ただし A-Open 全体の歴史では auxiliary-rich run が winning run を自動的に上回っていない。
したがってこれは

- local subskill teacher の reference asset
- mainline の中心 teacher ではない

と扱うべきである。

#### B. `answer_only_keep` の bit `271` 行

これは依然として使い道がある。
ただし用途は limited であり、full CoT teacher ではない。

使い道:

- boxed-only final answer stabilization
- short closure lane
- exact teacher の最終 commitment 補強

使ってはいけない用途:

- full reasoning teacher
- ambiguous hard tail の雑な exact 化

### 15.6 現時点で不足しているもの

ここまでの棚卸しから、まだ無いものはかなり限定できる。

#### A. persistent hard core 向け query-specific exact closure traces

既存 verified pool は広いが、hard watchlist の中心はまだ残っている。
...
ここは generic binary 増量ではなく、**query-specific closure を新規に書く必要がある**。

#### B. anti-`default 1` positive / negative contrast pack

`anti_default1_commit` は `v6` で `9` 例まで入ったが、watchlist 全体を覆うほど厚くはない。
既存資産はあるが、完成 corpus としては不足している。

必要なのは:

- hard row 近傍の exact positive pair
- `default 1` に逃げると誤る negative pair
- final query で divergence する contrastive closure

これは **新規 lane 作成が必要** である。

#### C. ready-made の promote-safe BIT mainline bundle

一番重要なのはここである。

現状あるのは

- public-safe base (`v4`, `04-08-16-14`)
- binary donor (`v6`)
- large audited raw pool (`1229 verified`)
- prompt-local / structured-byte candidate pools

であって、これらをまとめた **完成済みの本命 bundle** はまだ無い。

つまり v10 で必要なのは「新しい巨大 corpus を外から探すこと」ではない。
必要なのは、既存の usable asset を組み直した **new binary mainline bundle** である。

### 15.7 実務判断

BIT 大幅強化に向けた使用可能資産の整理として、現時点の判断は次である。

1. **既存資産は十分ある**
   - base / donor / verified raw pool / candidate pool は揃っている

2. **そのまま学習すべき未使用完成 corpus はない**
   - ready-made の mainline answer は存在しない

3. **新規に作るべきものは narrow でよい**
   - persistent hard core の query-specific closure
   - anti-`default 1` contrastive lane

4. **次の BIT 本命は「新規発掘」より「再編」**
   - `v4` / `04-08-16-14` を backbone とし
   - `v6` と `cuda-train-data-analysis-v1` の audited binary pool を donor にし
   - frontier 専用 lane だけを新規生成する

要するに、BIT 大幅強化に向けた最大の資産不足は data volume ではない。

**不足しているのは、既存 verified asset を frontier-specific teacher architecture に組み替えた corpus 設計である。**
