...
DEFAULT_OUTPUT_ROOT = VERSION_ROOT / 'outputs' / 'transformers_submission_v5_1'
...
DEFAULT_BENCHMARK_GREETING_PROMPT = 'こんにちは。日本語で一文だけ返答してください。'
DEFAULT_BENCHMARK_MAX_NEW_TOKENS = 64
MPS_ENV_KEYS = (
...
def run_simple_generation_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    seed_everything(args.seed)
    cache_dir = Path(args.cache_dir)
    base_model_path = resolve_base_model_path(
        base_model_path=args.base_model_path,
    ...
    benchmark_name = args.benchmark_name.strip() or 'simple_mps'
    benchmark_slug = re.sub(r'[^A-Za-z0-9._-]+', '_', benchmark_name).strip('_') or 'simple_mps'
    if args.output_dir:
        output_dir = Path(args.output_dir)
    else:
        timestamp = utc_now().replace(':', '').replace('+00:00', 'Z')
        output_dir = DEFAULT_OUTPUT_ROOT / 'benchmarks' / f'{timestamp}_{benchmark_slug}'
    output_dir.mkdir(parents=True, exist_ok=True)
    case_results_path = output_dir / 'case_results.jsonl'
    if case_results_path.exists():
        case_results_path.unlink()
    ...
    model.eval()
    model_ready_at = utc_now()
    model_load_seconds = time.perf_counter() - load_started_perf

    thinking_mode = str(args.thinking_mode)
    cases: list[dict[str, Any]] = [
        {
            'name': 'hello_ja_no_thinking',
            'prompt': args.greeting_prompt,
            'instruction': '',
            'enable_thinking': False,
            'max_new_tokens': args.max_new_tokens,
        },
    ]
    if thinking_mode == 'mixed':
        cases.append(
            {
                'name': 'hello_ja_thinking',
                'prompt': args.greeting_prompt,
                'instruction': '',
    ...
    if args.include_boxed_case:
        cases.append(
            {
                'name': 'boxed_math_thinking' if thinking_mode == 'mixed' else 'boxed_math_no_thinking',
                'prompt': args.boxed_prompt,
                'instruction': OFFICIAL_BOXED_INSTRUCTION,
                'enable_thinking': thinking_mode == 'mixed',
                'max_new_tokens': args.max_new_tokens,
            }
        )
    ...
    manifest = {
        ...
        'include_boxed_case': bool(args.include_boxed_case),
    ...
    save_json(output_dir / 'benchmark_manifest.json', manifest)

    results: list[dict[str, Any]] = []
    for case_index, case in enumerate(cases, start=1):
        user_content = build_eval_user_content(case['prompt'], case['instruction'])
        ...
        with torch.inference_mode():
            generated = model.generate(**generate_kwargs)
        generation_seconds = time.perf_counter() - generate_started_perf
        completed_at = utc_now()
        ...
        boxed_answer = extract_boxed_answer(generated_text) or extract_boxed_answer(decoded)
        toks_per_second = (
            float(generated_token_count) / generation_seconds if generation_seconds > 0.0 else None
        )
        ...
        case_result = {
            ...
            'generated_tokens_per_second': toks_per_second,
            'hit_max_new_tokens': generated_token_count >= int(case['max_new_tokens']),
            'generated_text': generated_text,
            'decoded_text': decoded,
        ...
        append_jsonl(case_results_path, case_result)
        print(
            json.dumps(
                {
                    'event': 'benchmark_case_done',
                    'case_index': case_index,
                    'case_count': len(cases),
                    'case_name': case['name'],
                    'generated_token_count': generated_token_count,
                    'generation_seconds': generation_seconds,
                    'generated_tokens_per_second': toks_per_second,
                    'hit_max_new_tokens': case_result['hit_max_new_tokens'],
                },
                ensure_ascii=False,
            ),
            flush=True,
    ...
    throughputs = [value for value in (row['generated_tokens_per_second'] for row in results) if value is not None]
    summary = {
        'benchmark_name': benchmark_name,
        'output_dir': str(output_dir),
        'adapter_dir': str(adapter_dir) if adapter_dir is not None else None,
        ...
        'model_load_seconds': model_load_seconds,
        'total_generation_seconds': sum(row['generation_seconds'] for row in results),
        'total_generated_tokens': sum(row['generated_token_count'] for row in results),
        'mean_generated_tokens_per_second': sum(throughputs) / len(throughputs) if throughputs else None,
        'min_generated_tokens_per_second': min(throughputs) if throughputs else None,
        'max_generated_tokens_per_second': max(throughputs) if throughputs else None,
        'created_at': utc_now(),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    benchmark_parser = subparsers.add_parser(
        ...
        help='Measure simple local generation throughput for the BF16 Transformers stack on the current device',
    )
    add_shared_model_args(benchmark_parser)
    benchmark_parser.add_argument('--adapter-dir', type=str, default='')
    benchmark_parser.add_argument('--output-dir', type=str, default='')
    benchmark_parser.add_argument('--benchmark-name', type=str, default='simple_mps')
    benchmark_parser.add_argument('--seed', type=int, default=0)
    benchmark_parser.add_argument('--max-new-tokens', type=int, default=DEFAULT_BENCHMARK_MAX_NEW_TOKENS)
    benchmark_parser.add_argument('--greeting-prompt', type=str, default=DEFAULT_BENCHMARK_GREETING_PROMPT)
    benchmark_parser.add_argument('--boxed-prompt', type=str, default=DEFAULT_PROMPT)
...
def main() -> None:
    ...
    if args.command == 'smoke-test':
        ...
        return
    if args.command == 'benchmark-simple-generation':
        run_simple_generation_benchmark(args)
        return
