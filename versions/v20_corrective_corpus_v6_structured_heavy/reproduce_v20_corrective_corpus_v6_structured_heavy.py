...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_heavy-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_heavy_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_heavy",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core structured-heavy exact-binary overlay, anti-default1 reinforcement on hard binary IDs, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-heavy corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, shifting extra overlay budget into structured-byte exact closure, "
            "and retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_heavy_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-heavy report",
        "",
        "## Purpose",
        "",
