# v20 corrective corpus v7-1 reassessment
...
つまり `v7-1` は「README 契約を満たす本命」ではなく、**README 契約のうち easy-family surface 側だけを局所的に直した run** です。
...
1. `v7-1` は validation だけ見ると最良だが、public は最良ではない
2. `v7-1` は proxy でも `v6` を下回り、`v4` すら超えていない
3. public best は依然として `v4` 系であり、local best と public best が一致していない

## What v7-1 actually changed
...
つまり `v7-1` の validation gain の主因は **numeral recovery** です。bit は net で大きく伸びていません。
...
どちらも binary で、しかも `verified_trace_ready` 側を含みます。これは重要です。`v7-1` は manual / answer-only の不安定 row だけでなく、**verified binary の一部でも後退**しています。
...
したがって `v7-1` は `v4` の public-safe balance を超えたとは言えません。

## Failure shape

`v7-1` proxy wrong rows は `22` 件です。
...
この集合は、以前から見えていた persistent hard core とほぼ重なっています。つまり `v7-1` は新しい frontier を切り開いていません。

### v7-1 proxy binary regressions are content regressions

`v7-1` で落ちた代表 row は、いずれも boxed 自体は保っています。
...
### 1. v7-1 is a validation-improving run, not a binary-improving run
...
### 2. v7-1 repeats the already-known proxy/public blind spot
...
が記録されていました。`v7-1` はこの blind spot を埋めていません。むしろ proxy 自体も `v6` より弱いので、public 0.84 は surprise ではなく **warning ignored** に近い結果です。

### 3. symbol is still weak, but symbol is not the decisive new failure here

`v7-1` proxy symbol は `24 / 32 = 0.7500` で、`v4` / `v6` と同水準です。
...
が中心ですが、`v4`, `v6`, `v7-1` の並びで **public best を選べていません**。このまま次の run を回しても、また local best を public loser と誤認する可能性が高いです。

必要なのは、既知の contradiction を集めた **public-risk mini gate** です。最低でも次を含めるべきです。

- `v4 > v6` だった hidden-sensitive rows
- `v6 > v7-1` で regression した verified binary rows
...
今までは aggregate score で見ていましたが、`v7-1` で分かったのは **aggregate が上がっても binary verified rows を落とせば public は負けうる** ということです。

### 3. Any v7-1-like mixed branch still needs source-aware token reuse

`v7` で確認済みですが、public-safe rows を donor と混ぜる branch では token stream 継承が source-aware でなければいけません。`v7-1` 自体は `v7` のような全面 collapse ではないものの、次の mixed mainline をやるならこの条件は引き続き mandatory です。
...
1. `v7-1` の validation `839 / 950` を理由に、そのまま次の主系列へ進めること
...
1. `v4 / v6 / v7-1` の contradiction rows だけで構成した small public-risk gate を作る
...
`v7-1` から得るべき結論は、「このまま次の実験へ進む」ではありません。
