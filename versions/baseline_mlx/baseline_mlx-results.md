...
- safe cleanup 版へ差し替えた waiter は現在 **8 本 live**、best-candidate 側も session script `best_submission_poller.sh` で稼働中で、`selection_manifest.json` は **2026-04-09 10:17:09** 更新を確認した。したがって現状は **train 2 本 + safe waiter/poller** の構成で、Stage1 完走後に Stage2 / suite / audit / export / ledger 更新まで自動で流せる状態に戻っている。
