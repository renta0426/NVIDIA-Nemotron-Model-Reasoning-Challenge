...
- Stage1 broad trunk は **`notebook-current` broad CSV** を使い、README 契約 (`rank<=32`, single adapter) を維持したまま **union 10 keys** (`broad 6 + attention 4`) を初期化している。初期 report は **`Iter 1: Val loss 1.677 (0.953s)`**、最初の optimizer 定点は **`Opt10 loss=0.892 / LR 2.195e-05 / It/sec 0.669 / Peak mem 80.897 GB`**。この速度帯なら stage1 全体は baseline fullrun v2 と同程度の時間帯で回せる見込みで、以後は throughput の維持と easy family drift を監視して Stage2 へ進める。
