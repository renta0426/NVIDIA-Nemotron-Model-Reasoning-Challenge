...
| `baseline-mlx-notebook-original-loss-v5` | `train --profile notebook-original --steps-per-report 1 --steps-per-report-step-unit optimizer` (partial diagnostic run) | `notebook-original` | `2907` | `5814` | `728` | `score=n/a`; `Opt1 loss=1.052 lr=0`; `Opt2 loss=1.916 lr=2.778e-06`; `Opt3 loss=1.305 lr=5.556e-06`; `Opt10 loss=0.599 lr=2.500e-05`; `peak_mem≈80.230 GB` | partial diagnostic completed | `baseline_mlx/outputs/nemotron_sft_lora_with_cot_v2_mlx_notebook_original_loss_v5/console.log` |
...
- 一方で loss の絶対値は HF notebook と一致しない。HF notebook HTML table の先頭は **`Step 10 = 10.750076`** だが、MLX diagnostic run の **`Opt 10 = 0.599`** だった。したがって現時点では **LR parity は取れたが、loss magnitude は HF と MLX の proxy 比較指標として使えない**。この差は base model / runtime / loss reporting semantics を含む複合差として扱う。
