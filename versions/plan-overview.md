...
3) vLLM/PEFT/TRL/MLX の現行ベストプラクティス
4) Apple Silicon/Mac Studio 側の実運用制約
を整理します。 その前提に基づいて「即実装レベル」の戦略一覧に落とします。了解。
「Mac Studio 512GB は探索・大量生成・高速アブレーションの主戦場」「Kaggle CUDA GPU (A6000 Pro 48GB/96GB 級) は有望レシピの BF16 再現・提出の正規ルート」 という制約で、top10 / 0.9+ を最も堅く狙うための、試すべき戦略を全部入りで、即実装レベルまで落として整理する。
...
1. 採点実コードに完全一致したローカル評価基盤を作る
   - 特に temperature=1.0, max_tokens=3584, max_model_len=4096, enable_thinking=True を正とする
   - これは README ではなく metric 実コード基準で進めるべき
   - ここをズラすと全部ズレる
...
5. Mac では MLX を「探索・生成・自己学習ループ」に全振りする
   - 大量 synthetic
   - teacher inference
   - rejection sampling
   - hard case mining
   - pilot LoRA / ablation
   - ここで勝った recipe だけを CUDA BF16 + PEFT に昇格する
...
README 側の説明値ではなく、実際に score() が呼ぶ値で評価する。
...
Eval 1
- 本番相当: 1 sample @ temp=1.0

Eval 2
- robustness: 8 sample @ temp=1.0
- 指標:
  - pass@1
  - pass@8
...
でも本番は temp=1.0 なので、長すぎる trace は事故源にもなる。
...
- BF16 + Transformers + PEFT による再学習
- Mac で勝った data ratio / weighting / style policy の忠実再現
- adapter_config.json / adapter_model.safetensors 生成
- vLLM / PeftModel.from_pretrained() / visible test smoke
- 毎日 5 submit の運用
...
6-5. MLX 学習アダプタは提出物ではなく、recipe 探索結果とみなす
MLX 学習成果物をそのまま vLLM 互換提出物に変換することを前提にしない。
現状は「Mac で勝ち筋を見つけ、CUDA BF16 + PEFT で再現したものだけを submission 候補にする」を正ルートにする。

必須
- Mac run ごとに data manifest / hyperparameter / seed / target_modules / weighting / style ratio を保存
- 有望 candidate は Kaggle CUDA で BF16 再学習
- submission artifact は CUDA run から作る
- adapter_config.json / adapter_model.safetensors / submission.zip を CUDA artifact で検証
- vLLM 実挙動確認と visible test smoke を CUDA artifact で通す
...
10-8. MLX アダプタ変換を本番提出の主ルートにする
ダメ。Mac で勝った recipe は CUDA BF16 + PEFT で再学習してから提出する。

11. 直ちに実装すべき ToDo

11-1. 今日やること
1. metric 完全複製 evaluator 実装
2. family parser 実装
3. stratified split A/B/C 作成
4. real canonical metadata 作成
5. format risk detector 実装
6. bit/unit/gravity/roman generator 実装
7. visible test を submission smoke 専用に隔離

11-2. 今週やること
1. sibling synth v1
2. teacher distilled short traces v1
3. baseline LoRA 3本 (Mac pilot)
4. final-span weighted loss 実験
5. format sharpening set 作成
6. hard example mining
7. 有望 recipe 1本を Kaggle BF16 + PEFT で再現
8. first Kaggle smoke submit
