...
「Kaggle GPUは提出・最終検証だけ」「主戦場は Mac Studio 512GB」 という制約で、top10 / 0.9+ を最も堅く狙うための、試すべき戦略を全部入りで、即実装レベルまで落として整理する。
...
5. Mac では MLX を「探索・生成・自己学習ループ」に全振り
   - 大量 synthetic
   - teacher inference
   - rejection sampling
   - hard case mining
...
Kaggle GPU を学習に使わないなら、Mac の価値はここ。
...
提出互換側
- Transformers + PEFT 形式への整形
- adapter_config.json
- adapter_model.safetensors
- PeftModel.from_pretrained() smoke test
...
6-5. MLX 学習アダプタは “変換・検証前提”
そのまま提出は危険。

必須
- PEFT 互換へ変換
- adapter_config.json を手で精査
- base_model_name_or_path 正しいか確認
- target modules 名が一致しているか確認
- load smoke test を必ず通す
...
- local overall +0.003 以上
- hard split +0.005 以上
- symbol/text fail 率明確改善
- format fail 大幅減
...
10-8. 互換確認なしに MLX アダプタ提出
ダメ。必ず PEFT load test。
