...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support_full-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support_full_bundle.jsonl"
...
def build_surface_completion(candidate: Candidate) -> str:
    ...
    if bucket == "surface_numeral_boxed":
        ...
        lines = [
            ...
            "Keep exact 8-bit ordering and every leading zero.",
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        if candidate.bucket == "binary_prompt_local_exact":
            variant_styles = ["exact_rule_commit", "short_closure_commit", "binary_zero_skill"]
        else:
        ...
        return [
            {
                ...
                "supervision_role": (
                    ...
                    else "lane1_exact_binary" if style != "anti_default1_commit" else "lane1b_anti_default1"
                ),
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_short_token_binary_support_full",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core structured-heavy exact-binary overlay, a short-closure rewrite lane, a token-skill auxiliary lane, "
            "anti-default1 reinforcement on hard binary IDs, and a small boxed-surface stabilizer lane."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-short-token-binary-support-full corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, preserving the structured short+token exact mix, and broadening the structured-byte answer-only "
            "support lane toward the full current-consensus pool while retaining README-consistent boxed-first surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_short_token_binary_support_full_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-short-token-binary-support-full report",
        "",
        "## Purpose",
        "",
        "This bundle keeps the v20 base snapshot,",
        "preserves the structured-heavy exact binary teacher mix with short-closure and token-skill support,",
        "broadens the structured-byte answer-only support lane for boxed 8-bit surface repair,",
        "adds anti-default1 supervision on measured hard binary IDs,",
        "and keeps README-consistent boxed-first answer extraction without mixing answer-only rows into the exact binary lane.",
        "",
