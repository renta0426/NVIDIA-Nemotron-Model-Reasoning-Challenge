...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_popular8_capped_tail_boost-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_popular8_capped_tail_boost_bundle.jsonl"
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    popular_operator_order = [
        operator
        for operator, _count in sorted(
            numeric_guess_operator_counts.items(),
            key=lambda item: (-item[1], item[0]),
        )[:NUMERIC_GUESS_POPULAR_OPERATOR_COUNT]
    ]
    popular_operator_rank = {operator: index + 1 for index, operator in enumerate(popular_operator_order)}
    for row in train_recommended_rows:
        row_id = row["id"]
        if str(row.get("template_subtype", "")).strip() != "numeric_2x2":
            continue
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_popular8_capped_tail_boost",
        "created_at": utc_now(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-numeric-guess-popular8-capped-tail-boost corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, reallocating more budget into prompt-local exact rows, "
            "adding both short-closure and token-skill lanes, adding a numeric answer-only support lane that keeps full tail-row and operator coverage while redirecting extra repetition toward the top-8 popular equation_numeric_guess operators described in the README strategy, with a cap on per-operator unique expansion, "
            "and strengthening the anti-default1 lane while retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_numeric_guess_popular8_capped_tail_boost_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-numeric-guess-popular8-capped-tail-boost report",
        "",
        "## Purpose",
        "",
        ...
        "adds a numeric answer-only support lane for equation_numeric rows with full tail-row coverage, operator coverage, and extra repetition concentrated on the popular top-8 operators with capped unique expansion,",
