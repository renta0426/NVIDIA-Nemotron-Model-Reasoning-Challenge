...
DEFAULT_TRACK_A_TRAIN_CSV = (
    REPO_ROOT / 'baseline' / 'cot' / 'phase1_assistant_only' / 'artifacts' / 'phase1_assistant_only_training_data.csv'
)
DEFAULT_TRAIN_CSV = DEFAULT_TRACK_A_TRAIN_CSV
...
def summarize_family_micro_smoke(
    *,
    ...
    summary = {
        'track': 'Track A. Plumbing-Hardened Generalist',
        'adapter_dir': str(adapter_dir),
        'output_dir': str(output_dir),
...
def run_track_a(args: argparse.Namespace) -> dict[str, Any]:
    train_args = argparse.Namespace(**vars(args))
    train_summary = train_lora(train_args)
    ...
    hypothesis = normalize_optional_text(args.hypothesis) or (
        'Assistant-only loss with verified/answer-only role separation should improve boxed final-answer stability '
        'without breaking the strong anchor families.'
    )
    combined = {
        'track': 'Track A. Plumbing-Hardened Generalist',
    ...
    save_json(adapter_dir / 'track_a_run_summary.json', combined)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            'NVIDIA Nemotron Track A submission work.'
        )
    )
    ...
    family_micro_smoke_parser = subparsers.add_parser(
        'family-micro-smoke',
        help='Run the curated Track A local family micro smoke set',
    )
    ...
    run_track_a_parser.add_argument('--zip-path', type=str, default=None)
    run_track_a_parser.add_argument('--hypothesis', type=str, default=None)
    add_inference_tuning_args(run_track_a_parser)
