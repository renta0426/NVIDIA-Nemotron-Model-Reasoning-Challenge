...
LOAD_IN_4BIT = True
AUG_COPIES = 1
HARD_TEMPLATE_EXTRA_COPIES = 1

OUTPUT_ROOT = Path("/kaggle/working/nemotron_v6_example_order_aug")
ADAPTER_DIR = OUTPUT_ROOT / "adapter"
SUBMISSION_ZIP = Path("/kaggle/working/submission_v6.zip")
...
def main() -> None:
    ...
    aug_df = expand_with_prompt_augmentations(
        tr_df,
        copies_per_row=AUG_COPIES,
        seed=SEED,
        hard_templates_extra_copies=HARD_TEMPLATE_EXTRA_COPIES,
    )
    stage1_df = pd.concat([tr_df, aug_df], ignore_index=True).sample(frac=1.0, random_state=SEED).reset_index(drop=True)
    stage2_df = tr_df.copy().reset_index(drop=True)

    print_split_summary("train_original", tr_df)
    print_split_summary("train_augmented_only", aug_df)
    print_split_summary("train_stage1_augmented", stage1_df)
    ...
    stage1_train_ds = build_sft_dataset(stage1_df, tokenizer, style="direct")
    stage2_train_ds = build_sft_dataset(stage2_df, tokenizer, style="direct")
    ...
    print("Stage 1 prompt-order augmentation training...")
    ...
    print("Stage 2 original-distribution polish...")
    ...
    summary = {
        ...
        "hard_template_extra_copies": HARD_TEMPLATE_EXTRA_COPIES,
        "max_length": MAX_LENGTH,
        "lora_r": LORA_R,
        "lora_alpha": LORA_ALPHA,
