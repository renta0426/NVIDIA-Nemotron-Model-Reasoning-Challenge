...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v1-results.md"
...
PROXY_ROW_LEVEL_PATH = REPO_ROOT / "A-Open-ProgressPrizePublication" / "leaderboard_proxy_eval_v20" / "artifacts" / "leaderboard_proxy_eval_row_level.csv"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v1_bundle.jsonl"
...
DEFAULT_REPEAT_COUNTS = {
    "bit_structured_byte_formula": 4,
    "bit_other": 3,
    "symbol_numeric_2x2": 2,
    "symbol_glyph_len5": 4,
}
...
def build_anchor_watchlist() -> list[dict[str, Any]]:
    rows = load_csv_rows(PROXY_ROW_LEVEL_PATH)
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v1",
        "created_at": utc_now(),
        ...
        "inputs": {
            ...
            "symbol_manual_audit_queue_v1": relative_to_repo(SYMBOL_MANUAL_QUEUE_PATH),
            "leaderboard_proxy_eval_row_level": relative_to_repo(PROXY_ROW_LEVEL_PATH),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "Includes the full v20 base snapshot plus repeated corrective overlay rows."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the highest-value corrective overlay corpus for the v20 Kaggle reproduction path, "
            "prioritizing bit_structured_byte_formula, bit_other, numeric_2x2, and glyph_len5."
        )
    )
    parser.add_argument("--run-name", default="corrective_overlay_default")
...
def main() -> None:
    ...
    candidates, diagnostics = load_ranked_candidates(validation_csv)
    ...
    report_lines = [
        "# v20 corrective overlay report",
        "",
        "## Purpose",
        "",
        "This overlay keeps the v20 base corpus intact and adds targeted corrective supervision for the",
        "highest-value Kaggle reproduction gaps identified from README-guided proxy slices.",
        "",
