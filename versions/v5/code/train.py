...
OUTPUT_ROOT = Path("/kaggle/working/nemotron_v5_fold_soup")
MERGED_ADAPTER_DIR = OUTPUT_ROOT / "merged_adapter"
SUBMISSION_ZIP = Path("/kaggle/working/submission_v5.zip")
...
def main() -> None:
    ...
    folds = make_stratified_folds(full_df, num_folds=NUM_FOLDS, seed=SEED)
    ...
    for fold_idx in range(NUM_FOLDS):
        val_df = folds[fold_idx].copy().reset_index(drop=True)
        train_parts = [folds[idx] for idx in range(NUM_FOLDS) if idx != fold_idx]
        tr_df = pd.concat(train_parts, ignore_index=True).sample(frac=1.0, random_state=SEED).reset_index(drop=True)

        print_split_summary(f"fold_{fold_idx + 1}_train", tr_df)
        print_split_summary(f"fold_{fold_idx + 1}_valid", val_df)
        ...
        trainer = Trainer(
            model=model,
            args=build_training_arguments(OUTPUT_ROOT / f"fold_{fold_idx + 1}" / "checkpoints"),
        ...
        eval_loss = float(eval_metrics.get("eval_loss", trainer.state.best_metric or 1.0))
        fold_scores.append(1.0 / max(eval_loss, 1e-6))

        fold_adapter_dir = OUTPUT_ROOT / f"fold_{fold_idx + 1}" / "adapter"
        fold_adapter_dir.mkdir(parents=True, exist_ok=True)
        trainer.model.save_pretrained(fold_adapter_dir, safe_serialization=True)
        adapter_dirs.append(fold_adapter_dir)
        fold_summaries.append(
            {
                ...
                "eval_loss": eval_loss,
                "adapter_dir": str(fold_adapter_dir),
            }
        )
    ...
    merge_weights = [score / total_score for score in fold_scores]
    merge_adapter_directories(adapter_dirs, MERGED_ADAPTER_DIR, weights=merge_weights)
