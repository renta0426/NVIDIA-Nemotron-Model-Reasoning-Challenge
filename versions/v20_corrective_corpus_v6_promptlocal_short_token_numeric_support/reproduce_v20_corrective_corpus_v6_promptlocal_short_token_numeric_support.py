...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_support-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_support_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_numeric_support",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "a narrow numeric answer-only support lane, and a heavier anti-default1 pack, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-numeric-support corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, reallocating more budget into prompt-local exact rows, "
            "adding both short-closure and token-skill lanes, adding a narrow numeric answer-only support lane, "
            "and strengthening the anti-default1 lane while retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_numeric_support_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-numeric-support report",
        "",
        "## Purpose",
        "",
        ...
        "adds a narrow numeric answer-only support lane for equation_numeric rows,",
