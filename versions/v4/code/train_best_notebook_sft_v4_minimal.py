    ...
    *,
    ...
    class TextDataset:
        ...
        def process(self, datum: dict[str, Any]) -> tuple[list[int], int, list[float]]:
            ...
            if raw_text is None:
                raise ValueError('minimal verified-best training expects pre-rendered `text` records')
...
def run_train_best(args: argparse.Namespace) -> None:
    import pandas as pd

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    candidate_id = normalize_optional_text(args.candidate_id) or 'v4_best_notebook_sft_bf16_official_ultralowlr_minimal_run1'
    pack_output_path = Path(args.pack_output_path) if args.pack_output_path else output_dir / f'{candidate_id}_train_pack.parquet'
    config_output_path = Path(args.config_output_path) if args.config_output_path else output_dir / f'{candidate_id}_inline_config.yaml'
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Minimal standalone trainer for the current verified-best BF16 notebook SFT recipe.')
    parser.add_argument(
        '--output-dir',
        default=str(VERSION_ROOT / 'outputs' / 'train' / 'best_notebook_sft_minimal_run1'),
        help='Output directory for manifests, metrics, config, and adapter artifacts.',
    )
    parser.add_argument('--candidate-id', default='v4_best_notebook_sft_bf16_official_ultralowlr_minimal_run1')
    parser.add_argument('--input-path', default=str(DEFAULT_TRAIN_CSV_PATH))
    parser.add_argument('--pack-output-path')
    parser.add_argument('--config-output-path')
    ...
    parser.add_argument('--execute', action='store_true', help='Run actual MLX BF16 LoRA training. Default is render-only smoke mode.')
    return parser
