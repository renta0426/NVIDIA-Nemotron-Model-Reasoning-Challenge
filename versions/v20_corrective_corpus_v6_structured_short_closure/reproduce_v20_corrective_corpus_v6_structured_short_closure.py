...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_short_closure-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_short_closure_bundle.jsonl"
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        if candidate.bucket == "binary_prompt_local_exact":
            variant_styles = ["exact_rule_commit", "short_closure_commit"]
        else:
            variant_styles = ["exact_rule_commit", "exact_closure_commit", "short_closure_commit"]
        ...
        rows.append(
            {
                "assistant_style": style,
                "supervision_role": (
                    "lane2_short_closure" if style == "surface_short_closure" else "lane4_surface_stabilizer"
                ),
                "completion_text": (
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_short_closure",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core structured-heavy exact-binary overlay, a short-closure rewrite lane for binary and easy-family rows, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-short-closure corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, shifting extra budget into structured-byte exact closure and adding a short-closure rewrite lane, "
            "while retaining a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_short_closure_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-short-closure report",
        "",
        "## Purpose",
        "",
        ...
        "adds a short-closure rewrite lane for binary and easy-family rows,",
