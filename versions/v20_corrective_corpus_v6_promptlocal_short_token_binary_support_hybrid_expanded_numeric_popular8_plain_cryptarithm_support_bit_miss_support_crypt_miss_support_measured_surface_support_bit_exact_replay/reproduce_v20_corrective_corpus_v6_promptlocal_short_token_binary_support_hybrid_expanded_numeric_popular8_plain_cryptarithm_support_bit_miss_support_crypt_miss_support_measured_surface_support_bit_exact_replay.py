...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_miss_support_crypt_miss_support_measured_surface_support_bit_exact_replay-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_miss_support_crypt_miss_support_measured_surface_support_bit_exact_replay_bundle.jsonl"
...
BIT_V4_PARTIAL_EXACT_REPLAY_IDS = {
    "000b53cf",
    "00890aff",
    "012fb81b",
    "01e09228",
...
BIT_V4_PARTIAL_EXACT_REPLAY_BONUS_REPEAT = 2
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        ...
        if candidate.row["id"] in BIT_V4_PARTIAL_EXACT_REPLAY_IDS:
            target_count += BIT_V4_PARTIAL_EXACT_REPLAY_BONUS_REPEAT
        rows = [
            {
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    for row in train_recommended_rows:
        ...
        if bucket == "binary_structured_exact_core" and not (
            ...
            or mandatory_binary
            or row_id in BIT_V4_PARTIAL_EXACT_REPLAY_IDS
        ):
            continue
        ...
        if row_id in BIT_V4_PARTIAL_EXACT_REPLAY_IDS:
            source_tags.add("v4_live_partial_bit_exact_replay")
        ...
        priority_key = (
            ...
            -(1 if row_id in BIT_V4_PARTIAL_EXACT_REPLAY_IDS else 0),
    ...
    diagnostics = {
        ...
        "v4_live_partial_bit_exact_replay_ids": sorted(BIT_V4_PARTIAL_EXACT_REPLAY_IDS),
...
def select_candidates(candidates: list[Candidate], bucket_limits: dict[str, int]) -> list[Candidate]:
    ...
    for candidate in candidates:
        if candidate.row["id"] not in BIT_V4_PARTIAL_EXACT_REPLAY_IDS:
            continue
        if candidate.bucket not in {
            "binary_structured_exact_core",
            "binary_logic_exact",
        ...
        if counts[candidate.bucket] >= bucket_limits[candidate.bucket]:
            continue
        add_candidate(candidate)
        if candidate.bucket == "binary_structured_exact_core":
            structured_family_counts[binary_family_key(candidate.row)] += 1
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_miss_support_crypt_miss_support_measured_surface_support_bit_exact_replay",
        "created_at": utc_now(),
