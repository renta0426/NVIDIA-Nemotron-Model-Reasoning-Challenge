...
### v143-v145 corrected trace-twin route
...
`v143 / v144 / v145` はその quotas を **`single-adapter-fusion-v40` base に固定**し直した clean corrected branch。
...
| `v145` | corrected `v75` = `v143` + leading-zero startwith focus | `2048 rows`, `128 iters`; `final val 0.322 -> 0.318` | binary60 **`7/60` official, `3/60` exact**。`bit_structured_byte_formula` は **`0/14` official/exact**。`leading_zero_retention_rate = 0.1667`。完走 **4403.1s** | 不採用 |

解釈:

- train loss / final val だけ見ると `v143 / v144 / v145` はかなり良かったが、README binary60 では **どれも `v40` binary branch を全く更新できなかった**。
- failure mode はほぼ同一で、format bucket は
  - `v143`: **`numeric_fallback 55 / boxed 5`**
  - `v144`: **`numeric_fallback 54 / boxed 6`**
  - `v145`: **`numeric_fallback 52 / boxed 8`**
  だった。平均 raw output 長も **約 1.90〜1.95 万文字**まで伸び、README decode としては遅すぎる。
- `v145` は official `7/60` まで少し伸び、`leading_zero_retention_rate` も **`0.1667`** まで上がったが、**exact は `3/60` のまま**で、gain は `bit_other` の leading-zero collapse 側に限られた。
- structured は
  - `v143`: **`5f29ae58: 00000000 -> 0`** の collapse 1 件で official のみ
  - `v144`: official / exact とも **0**
  - `v145`: official / exact とも **0**
  だった。つまり leading-zero focus を入れても、**`bit_structured_byte_formula` exact は 1 件も増えなかった**。
- したがって **corrected trace-twin route (`v73/v74/v75` intended idea)** は、recipe mismatch を直しても **actual score / exactness / runtime の 3 点で不採用**。
...
- `mac_workspace/v1/outputs/resume_v40_to_top8_fusion_v145_lr2p5e6_ep025/prepare_manifest.json`
