...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid",
        "created_at": utc_now(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-binary-support-hybrid corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the prompt-local full support base, then adding a separate structured-byte answer-only support lane "
            "while retaining short-closure, token-skill, anti-default1, and minimal boxed-surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_binary_support_hybrid_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-binary-support-hybrid report",
        "",
        "## Purpose",
        "",
        ...
        "adds a full prompt-local bit answer-only support lane plus a narrow structured-byte answer-only support lane,",
