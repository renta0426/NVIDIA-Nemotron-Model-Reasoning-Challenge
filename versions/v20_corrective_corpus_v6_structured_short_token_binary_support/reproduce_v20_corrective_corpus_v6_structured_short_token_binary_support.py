...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_short_token_binary_support",
        "created_at": utc_now(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-short-token-binary-support corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, preserving the structured short+token exact mix, and adding a separate structured-byte answer-only "
            "support lane while retaining README-consistent boxed-first surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_short_token_binary_support_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-short-token-binary-support report",
        "",
        "## Purpose",
        "",
        ...
        "adds a separate structured-byte answer-only support lane for boxed 8-bit surface repair,",
