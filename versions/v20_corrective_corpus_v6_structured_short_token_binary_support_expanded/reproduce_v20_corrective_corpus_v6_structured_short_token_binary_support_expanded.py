...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support_expanded-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_short_token_binary_support_expanded_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_short_token_binary_support_expanded",
        "created_at": utc_now(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-short-token-binary-support-expanded corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, preserving the structured short+token exact mix, and widening the separate structured-byte answer-only "
            "support lane while retaining README-consistent boxed-first surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_short_token_binary_support_expanded_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-short-token-binary-support-expanded report",
        "",
        "## Purpose",
        "",
        ...
        "widens the separate structured-byte answer-only support lane for boxed 8-bit surface repair,",
