...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v3_mainline-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v3_mainline_bundle.jsonl"
...
RESULT_NEW_VALIDATION_PATH = RESULT_ROOT / "results-v20-kaggle-v20-sft-repro-0414-104905" / "validation.csv"
RESULT_BASE_RESULTS_PATH = RESULT_ROOT / "results-v20" / "results.csv"
RESULT_NEW_RESULTS_PATH = RESULT_ROOT / "results-v20-kaggle-v20-sft-repro-0414-104905" / "results.csv"
RESULT_BASE_PROXY_PATH = RESULT_ROOT / "leaderboard_proxy_eval_v20" / "artifacts" / "leaderboard_proxy_eval_row_level.csv"
RESULT_NEW_PROXY_PATH = RESULT_ROOT / "leaderboard_proxy_eval_kaggle-v20-sft-repro-0414-104905" / "artifacts" / "leaderboard_proxy_eval_row_level.csv"
...
DEFAULT_BUCKET_LIMITS = {
    "binary_structured_core": 192,
    "binary_other_light": 80,
    "guardrail_numeral_subtractive": 12,
    "guardrail_gravity_fragile": 12,
    "guardrail_unit_fragile": 12,
    "guardrail_cryptarithm_symbolic": 12,
    "guardrail_symbol_prefix": 4,
}

DEFAULT_REPEAT_COUNTS = {
    "binary_structured_core": 4,
    "binary_other_light": 2,
    "guardrail_numeral_subtractive": 2,
    "guardrail_gravity_fragile": 1,
    "guardrail_unit_fragile": 1,
    "guardrail_cryptarithm_symbolic": 1,
    "guardrail_symbol_prefix": 1,
}
...
MEASURED_MANDATORY_IDS = {
    "binary_structured_core": {"0520a6ec", "0a50c4a8", "59c78e51", "fa67da07"},
    "binary_other_light": {"8e5d6fe6", "b9500f41", "c30a782a"},
    "guardrail_numeral_subtractive": {"081ad3f3"},
    "guardrail_gravity_fragile": {"0fd289d8"},
    "guardrail_unit_fragile": {"626d6c5f"},
    "guardrail_cryptarithm_symbolic": {"065abaf6", "0dcfd566"},
    "guardrail_symbol_prefix": {"d9bedb64"},
}
...
def load_measured_signals() -> dict[str, Any]:
    ...
    new_validation = read_csv_map(RESULT_NEW_VALIDATION_PATH)
    base_proxy = read_csv_map(RESULT_BASE_PROXY_PATH)
    new_proxy = read_csv_map(RESULT_NEW_PROXY_PATH)

    validation_improved: set[str] = set()
    validation_regressed: set[str] = set()
    for row_id in sorted(set(base_validation) & set(new_validation)):
        base_correct = base_validation[row_id]["correct"] == "True"
        new_correct = new_validation[row_id]["correct"] == "True"
    ...
    for row_id in sorted(set(base_proxy) & set(new_proxy)):
        base_correct = base_proxy[row_id]["is_correct"] == "True"
        new_correct = new_proxy[row_id]["is_correct"] == "True"
        if (not base_correct) and new_correct:
        ...
        if (
            new_proxy[row_id].get("template_subtype") == "numeric_2x2"
            and answer_has_symbolic_prefix(new_proxy[row_id].get("gold_answer", ""))
        ):
    ...
    return {
        ...
        "symbol_prefix_pool": symbol_prefix_pool,
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    guardrail_specs = [
        (
            "guardrail_numeral_subtractive",
            "numeral",
            lambda answer: any(token in str(answer) for token in ("IV", "IX", "XL", "XC")),
        ),
        ("guardrail_gravity_fragile", "gravity", lambda answer: True),
        ("guardrail_unit_fragile", "unit_conversion", lambda answer: True),
        ("guardrail_cryptarithm_symbolic", "cryptarithm_deduce", answer_is_symbolic_crypt),
    ]
    for bucket, category_name, predicate in guardrail_specs:
    ...
    symbol_prefix_ids = sorted(
        set(MEASURED_MANDATORY_IDS["guardrail_symbol_prefix"]) | set(measured["symbol_prefix_pool"])
    )[: bucket_limits["guardrail_symbol_prefix"]]
    for rank, row_id in enumerate(symbol_prefix_ids):
        proxy_row = measured["new_proxy"].get(row_id, {})
        source_tags = {"guardrail_symbol_prefix", "proxy_symbol_prefix"}
        if row_id in MEASURED_MANDATORY_IDS["guardrail_symbol_prefix"]:
            source_tags.add("measured_regression")
        priority_key = (
            -(1 if row_id in MEASURED_MANDATORY_IDS["guardrail_symbol_prefix"] else 0),
            rank,
            row_id,
        )
        candidate = make_candidate(
            row_id=row_id,
            bucket="guardrail_symbol_prefix",
            train_prompts=train_prompts,
            categories=categories,
            source_tags=source_tags,
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v3_mainline",
        "created_at": utc_now(),
        ...
        "inputs": {
            ...
            "new_validation": relative_to_repo(RESULT_NEW_VALIDATION_PATH),
            "base_proxy_row_level": relative_to_repo(RESULT_BASE_PROXY_PATH),
            "new_proxy_row_level": relative_to_repo(RESULT_NEW_PROXY_PATH),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a teacher-correct-only binary+guardrail overlay."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a corrected v3 mainline corpus for the v20 Kaggle reproduction path, "
            "removing snapshot teacher errors and keeping only teacher-correct overlay examples."
        )
    )
    parser.add_argument("--run-name", default="v3_mainline_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v3 mainline report",
        "",
        "## Purpose",
        "",
        "This bundle removes the known metric-wrong base snapshot problem from v20,",
        "filters overlay candidates to teacher-correct rows only,",
        "and reallocates token budget toward binary gains while keeping light guardrails for fragile easy families.",
        "",
