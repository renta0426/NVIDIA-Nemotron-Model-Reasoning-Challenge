...
BEST_RUN_NAME = (
    "nemotron_sft_lora_with_cot_v2_mlx_stagefreeze_v2_stage25_attention_qkvo_reanchor_"
    "textv130_bin40proxyo20p5s15_grav15_unit15_rowselect_lr8e6_len1024_from_reanchor1024_v1"
)
BEST_DATASET_STEM = "stage25_text_verified130_binary40proxyo20p5s15_grav15_unit15_rowselect_v1"
FULL_REPRO_RUN_NAME = (
    "nemotron_sft_lora_with_cot_v2_mlx_stagefreeze_v2_stage25_attention_qkvo_reanchor_"
    "textv130_bin40proxyo20p5s15_grav15_unit15_rowselect_lr8e6_len1024_from_reanchor1024_repro_v1"
)
...
def verify_readme_contract(prepare_manifest: dict[str, Any]) -> None:
    ...
    require(
        ...
        "Binary40 best run does not use rank-32 LoRA as required by README.md",
    )
...
def verify_best_run() -> dict[str, Any]:
    ...
    require(
        dataset_summary.get("requested_rows", {}).get("binary_bit_permutation_rows") == 5,
        "Binary40 repro expects the original permutation-inclusive row mix",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verify = subparsers.add_parser("verify", help="Verify the tracked binary40 best run and optionally write a summary.")
    verify.add_argument(
        "--output-root",
        type=Path,
        default=REPO_ROOT / "baseline_mlx" / "outputs" / "binary40_local_best_repro_verify_v1",
    )
    verify.set_defaults(func=command_verify)

    export_existing = subparsers.add_parser(
        "export-existing",
        help="Re-run audit/export on the completed binary40 best run and regenerate submission.zip.",
    )
    export_existing.add_argument(
        "--output-root",
        type=Path,
        default=REPO_ROOT / "baseline_mlx" / "outputs" / "binary40_local_best_repro_export_v1",
    )
    ...
    full_reproduce = subparsers.add_parser(
        "full-reproduce",
        help="Rebuild the binary40 dataset, rerun the short train, and postprocess the full suite.",
    )
