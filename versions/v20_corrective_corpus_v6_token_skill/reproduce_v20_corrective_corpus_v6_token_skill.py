...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_token_skill-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_token_skill_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_token_skill",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core exact-binary multi-lane overlay, modernized token-skill auxiliary supervision, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 token-skill corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, adding v6-core exact-binary overlays plus a token-skill auxiliary lane, "
            "and retaining a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_token_skill_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 token-skill report",
        "",
        "## Purpose",
        "",
        "This bundle keeps the v20 base snapshot,",
        "adds the v6-core exact binary teacher lanes for structured, logic, permutation, and prompt-local failures,",
