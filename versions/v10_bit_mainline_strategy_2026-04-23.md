...
| `v9` | `823/950 = 0.8663` | `176/200 = 0.8800` | `76/92 = 0.8261` | pending | matching auxiliary restore, measured negative |

この表から v10 の前提は 5 つに絞られる。
...
5. `v9` は matching auxiliary を戻しても BIT frontier は動かず、むしろ numeral surface を大きく壊しうることを示した

### 3.2 失敗から分かった設計上の禁止事項

`v7`, `v7-1`, `v8` を通すと、mainline でやってはいけないことは明確である。

`v9` を加えると、その禁止事項はさらに強くなる。
...
5. matching / token-skill の broad 復帰を mainline の主レバーにすること
   - `v9` は A-Open-style matching auxiliary を `543` 行戻したが、proxy binary は `76 / 92` まで落ち、validation bit も `150 / 169` から前進しなかった
   - そのうえ numeral は `133 / 149` まで崩れ、boxed `0` drift を大量に起こした
...
5. matching / token-skill の broad 復帰
   - `v9` の実測で mainline EV は低いと分かった
   - もし再度試すなら v10 の後段に置く diagnostic side branch に限る
...
`v9` が追加で示したのは、README の auxiliary 思想そのものが誤りだということではない。誤りなのは、それを **official `0.87` を狙う mainline の第一優先レバー** と見なすことである。
