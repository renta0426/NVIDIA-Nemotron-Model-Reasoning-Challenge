...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_mixed-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_mixed_bundle.jsonl"
...
NUMERIC_GUESS_POPULAR_BONUS_REPEAT = 1
NUMERIC_GUESS_VERY_POPULAR_BONUS_REPEAT = 2
NUMERIC_GUESS_FALLBACK_BONUS_REPEAT = 1
NUMERIC_GUESS_TAIL_POOL_MAX = 3
NUMERIC_GUESS_POPULAR_UNIQUE_CAP = 5
NUMERIC_GUESS_FALLBACK_UNIQUE_CAP = 2
NUMERIC_GUESS_MIX_UNIQUE_LIMIT = 48
NUMERIC_DEDUCE_MIX_UNIQUE_LIMIT = 16
NUMERIC_DEDUCE_BASE_REPEAT_COUNT = 3
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    numeric_deduce_support_candidates = 0
    ...
    for row in train_recommended_rows:
        ...
        if str(row.get("selection_tier", "")).strip() != "answer_only_keep":
            continue
        if row_id in MEASURED_MANDATORY_IDS.get("surface_symbol_prefix", set()):
            continue
        if (REASONING_DIR / f"{row_id}.txt").exists() is False:
            continue
        category = categories.get(row_id, "")
        ...
        if category == "equation_numeric_guess":
            ...
            source_tags.add("equation_numeric_deduce")
        if current_proxy_failed:
            source_tags.add("current_proxy_fail")
        if current_validation_failed:
            source_tags.add("current_validation_fail")
        ...
        if candidate is not None:
            ...
            if category == "equation_numeric_guess":
                ...
                numeric_deduce_support_candidates += 1
            candidates.append(candidate)
    ...
    diagnostics = {
        ...
        "numeric_deduce_support_candidates": numeric_deduce_support_candidates,
...
def select_candidates(candidates: list[Candidate], bucket_limits: dict[str, int]) -> list[Candidate]:
    ...
    numeric_guess_candidates = [candidate for candidate in numeric_candidates if candidate.row["category"] == "equation_numeric_guess"]
    numeric_deduce_candidates = [candidate for candidate in numeric_candidates if candidate.row["category"] == "equation_numeric_deduce"]
    numeric_guess_target = min(NUMERIC_GUESS_MIX_UNIQUE_LIMIT, bucket_limits[numeric_bucket])
    numeric_deduce_target = min(
        NUMERIC_DEDUCE_MIX_UNIQUE_LIMIT,
        max(0, bucket_limits[numeric_bucket] - numeric_guess_target),
    )
    numeric_guess_selected = 0
    numeric_deduce_selected = 0

    def add_numeric_candidate(candidate: Candidate) -> bool:
        nonlocal numeric_guess_selected, numeric_deduce_selected
        before = counts[numeric_bucket]
        add_candidate(candidate)
        if counts[numeric_bucket] == before:
            return False
        if candidate.row["category"] == "equation_numeric_guess":
            numeric_guess_selected += 1
        else:
            numeric_deduce_selected += 1
        return True

    for candidate in numeric_deduce_candidates:
        if numeric_deduce_selected >= numeric_deduce_target:
            break
        add_numeric_candidate(candidate)

    numeric_guess_by_operator: dict[str, list[Candidate]] = defaultdict(list)
    for candidate in numeric_guess_candidates:
        if candidate.row["category"] != "equation_numeric_guess":
            continue
    ...
    for operator in operator_order:
        if numeric_guess_selected >= numeric_guess_target:
            break
        add_numeric_candidate(numeric_guess_by_operator[operator][0])
    ...
    for operator in fallback_operator_order:
        while numeric_guess_selected < numeric_guess_target:
            ...
            for candidate in numeric_guess_by_operator.get(operator, []):
                if add_numeric_candidate(candidate):
    ...
    for candidate in numeric_guess_candidates:
        ...
        add_numeric_candidate(candidate)
    for candidate in numeric_deduce_candidates:
        if numeric_deduce_selected >= numeric_deduce_target or counts[numeric_bucket] >= bucket_limits[numeric_bucket]:
            break
        add_numeric_candidate(candidate)
...
def build_overlay_rows(selected: list[Candidate], repeat_counts: dict[str, int]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for candidate in selected:
        ...
        if candidate.bucket == "surface_numeric_answer_only":
            if candidate.row["category"] == "equation_numeric_deduce":
                base_variant_count = NUMERIC_DEDUCE_BASE_REPEAT_COUNT
            elif candidate.row["category"] == "equation_numeric_guess":
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_mixed",
        "created_at": utc_now(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-binary-support-hybrid-expanded-numeric-mixed corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the prompt-local full support base, widening the structured-byte answer-only support lane, "
            "and adding a numeric mixed lane with 48 popular-first guess rows plus 16 deduce rows while retaining short-closure, "
            "token-skill, anti-default1, and minimal boxed-surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_mixed_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-binary-support-hybrid-expanded-numeric-mixed report",
        "",
        "## Purpose",
        "",
