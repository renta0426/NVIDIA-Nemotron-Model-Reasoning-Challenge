...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_structured_token_skill-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_structured_token_skill_bundle.jsonl"
...
DEFAULT_REPEAT_COUNTS = {
    "binary_structured_exact_core": 3,
    "binary_logic_exact": 3,
    "binary_permutation_exact": 3,
    "binary_prompt_local_exact": 2,
    "surface_numeral_boxed": 2,
    "surface_cipher_boxed": 2,
    "surface_unit_tail": 2,
    "surface_symbol_prefix": 2,
    "easy_gravity_fragile": 2,
}
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        if candidate.bucket == "binary_prompt_local_exact":
            variant_styles = ["exact_rule_commit", "binary_zero_skill"]
        else:
            variant_styles = ["exact_rule_commit", "exact_closure_commit", "binary_zero_skill"]
    ...
    variant_styles = ["surface_boxed_tail", "surface_token_skill"]
        ...
        rows.append(
            {
                "assistant_style": style,
                "supervision_role": (
                    "lane3_token_skill" if style == "surface_token_skill" else "lane4_surface_stabilizer"
                ),
                "completion_text": (
                    build_surface_token_skill_completion(candidate)
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_structured_token_skill",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "plus a v6-core structured-heavy exact-binary overlay, modernized token-skill auxiliary supervision, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 structured-token-skill corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, shifting extra budget into structured-byte exact closure plus a token-skill auxiliary lane, "
            "and retaining a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_structured_token_skill_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 structured-token-skill report",
        "",
        "## Purpose",
        "",
        "This bundle keeps the v20 base snapshot,",
        "adds a structured-heavy exact binary teacher mix with more structured-byte exact closure budget,",
        "adds a token-skill auxiliary lane for boxed closure, prefix retention, and binary leading-zero exactness,",
