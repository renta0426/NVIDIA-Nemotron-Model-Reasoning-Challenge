# v20_corrective_corpus_v52_bit_binary_affine_boolean4_numeric_quote_cipher_unknown123_hard4

- created_at: 2026-04-28T16:51:05.502295+00:00
...
- analysis basis: combine `cuda-train-data-analysis-v1/reports/67_aopen_symbol_wall_breakthrough_assessment.md` for low-ratio numeric operator support and `reports/06_text_unknown_notes.md` for safe monoalphabetic completion, but isolate the `"`-only numeric slice and widen cipher to unknown-char `{1,2,3}` with `hard_score >= 4`.
- local target: current best local300 `0.846667` -> aim for `> 0.9` by preserving v34's exact affine/boolean4 bit core and testing whether the quote-only numeric lane benefits from the extra unknown=3 cipher rows.
- status: bundle generated; model score not yet measured.
- planned run name: `v20_mlx_v52_bit_binary_affine_boolean4_numeric_quote_cipher_unknown123_hard4_mlxdir_mb1_nobc_ckpt20`
...
- Increase both local numeric/cipher rescue repeats, then add only the `"` numeric operator rows and the harder cipher slice widened to `unknown_char_count in {1,2,3}`.
- Use this branch as the unknown123 counterpart to v50 so the quote lane can be compared directly against the star-only matrix.
...
- v52_binary_answer_only_affine_boolean4_numeric_quote_cipher_unknown123_hard4: 418
- v52_binary_manual_affine_boolean4_numeric_quote_cipher_unknown123_hard4: 347
- v52_binary_verified_affine_boolean4_numeric_quote_cipher_unknown123_hard4: 7605
- v52_cipher_numeric_quote_cipher_unknown123_hard4: 197
- v52_numeric_guess_quote_cipher_unknown123_hard4: 58
...
- path: A-Open-ProgressPrizePublication/nemotron/training/sft/MLX/v20_corrective_corpus_v52_bit_binary_affine_boolean4_numeric_quote_cipher_unknown123_hard4_bundle.jsonl
- base_examples: 7830
- overlay_examples: 8625
- total_examples: 16455
- total_steps: 515
- total_tokens: 30706623
- max_seq_len: 7971
