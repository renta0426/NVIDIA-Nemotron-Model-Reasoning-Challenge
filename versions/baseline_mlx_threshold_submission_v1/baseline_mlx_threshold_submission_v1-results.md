...
- README contract: `max_tokens=7680`, `top_p=1.0`, `temperature=0.0`, `max_num_seqs=64`, `gpu_memory_utilization=0.85`, `max_model_len=8192`, `max_lora_rank=32`
...
- existing_peft_export_ready: `True`
- existing_validation_valid: `True`
- reproduced_peft_export_ready: `True`
- reproduced_validation_valid: `True`
- exportability_note: the checked-in generated threshold summary still shows historical `existing_audit_status` / `reproduced_audit_status`; the active wrapper now also surfaces `existing_peft_export_ready` / `reproduced_peft_export_ready`, and those booleans together with `existing_validation_valid` / `reproduced_validation_valid` are the live README-facing exportability signals. The retired `existing_export_valid` alias is no longer an active field name.
...
- existing generated `threshold_submission_summary.{json,md}` artifacts under `baseline_mlx/outputs/threshold_submission_*` predate the README `gpu_memory_utilization = 0.85` sync in the wrapper code, so this versioned results file is the authoritative Git-visible contract ledger until those summaries are regenerated after shell recovery
- once regenerated, those summary artifacts will also stamp `summary_schema_version = 2`, `readme_contract_state`, `readme_contract_verified_from_readme_file = true`, and the explicit `existing_peft_export_ready` / `reproduced_peft_export_ready` fields so README contract completeness and live-README revalidation are machine-visible
