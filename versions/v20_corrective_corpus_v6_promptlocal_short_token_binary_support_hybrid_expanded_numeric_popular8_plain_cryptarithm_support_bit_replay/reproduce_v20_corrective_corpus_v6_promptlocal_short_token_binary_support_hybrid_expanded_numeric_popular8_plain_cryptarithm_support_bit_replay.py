...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_replay-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_replay_bundle.jsonl"
...
BIT_MEASURED_REPLAY_BONUS_REPEAT = 2
...
def build_supervision_variants(candidate: Candidate, base_variant_count: int) -> list[dict[str, str]]:
    if candidate.bucket.startswith("binary_"):
        ...
        if candidate.row["id"] in BIT_MEASURED_REPLAY_IDS:
            target_count += BIT_MEASURED_REPLAY_BONUS_REPEAT
        rows = [
            {
        ...
        if bonus_variant_count:
            ...
            for index in range(bonus_variant_count):
                ...
                rows.append(boosted)
        return rows
    if candidate.row["id"] in BIT_MEASURED_REPLAY_IDS and candidate.bucket.startswith("surface_binary_"):
        base_variant_count += BIT_MEASURED_REPLAY_BONUS_REPEAT
...
def build_budgeted_candidates(bucket_limits: dict[str, int]) -> tuple[list[Candidate], dict[str, Any]]:
    ...
    for row in train_recommended_rows:
        ...
        if bucket == "binary_structured_exact_core" and not (
            ...
            or mandatory_binary
            or row_id in BIT_MEASURED_REPLAY_IDS
        ):
            continue
        ...
        if row_id in BIT_MEASURED_REPLAY_IDS:
            source_tags.add("v4_live_partial_bit_replay")
...
def select_candidates(candidates: list[Candidate], bucket_limits: dict[str, int]) -> list[Candidate]:
    ...
    for candidate in candidates:
        if candidate.row["id"] not in BIT_MEASURED_REPLAY_IDS:
            continue
        if candidate.bucket not in {
            "binary_structured_exact_core",
            "binary_logic_exact",
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_replay",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "and extra measured replay repeats on the current v4 partial bit failures."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-binary-support-hybrid-expanded-numeric-popular8-plain-cryptarithm-support-bit-replay corrective corpus for the v20 Kaggle reproduction path, "
            ...
            "and adding measured replay repeats for the current v4 partial bit failures while retaining short-closure, "
            "token-skill, anti-default1, and minimal boxed-surface stabilizers."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_binary_support_hybrid_expanded_numeric_popular8_plain_cryptarithm_support_bit_replay_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-binary-support-hybrid-expanded-numeric-popular8-plain-cryptarithm-support-bit-replay report",
        "",
        "## Purpose",
        "",
        ...
        "and adds extra replay pressure on the current v4 partial bit failures,",
