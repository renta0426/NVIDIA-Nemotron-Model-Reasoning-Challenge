...
Scope: `versions/plan-overview.md` のうち、v2 で固定したデータ・学習基盤を使って **実際に勝ち筋のある SFT 実験列と submission 直結の packaging ループ** を完成させる  
Purpose: v2 の基盤版を、**real teacher trace / format-aware weighted SFT / MLX-to-PEFT bridge / candidate promotion** まで含む実戦版へ引き上げる
...
- MLX 学習成果物をそのまま PEFT submission に流せる状態でもない
...
6. MLX adapter を PEFT 互換 submission artifact へ変換する  
7. v1 evaluator で serious gate を通す candidate selection loop を固定する  
...
### 1-6. 実行環境は Mac 固定、かつ 512GB RAM を使い切る
...
v3 は **MLX で学習しても、提出物は PEFT 互換で出す**。
...
14. `bridge_verification_v3.json`  
15. `submission_manifest_v3.json`  
16. `submission_v3.zip`  
17. `training_command_book_v3.txt`  
18. `promotion_rules_v3.txt`  
19. v3 tests  
20. `versions/v3/code/train.py`
...
- Mac-to-PEFT の end-to-end bridge
- correction / format / preference / RFT accept pool の共通 schema
- serious gate を通した candidate promotion の台帳
...
│   ├── train/
│   │   ├── sft_stage_a_weighted.yaml
│   │   ├── sft_stage_a_weighted_alpha64.yaml
│   │   ├── sft_stage_b_weighted.yaml
│   │   ├── sft_stage_b_answer_bias.yaml
│   │   └── sft_stage_b_a6000_compat.yaml
│   ├── preference/
│   │   ├── dpo_format_pairs.yaml
│   │   └── dpo_correction_pairs.yaml
│   └── package/
│       └── peft_bridge_smoke.yaml
├── data/
│   ├── processed/
...
6. weighted SFT 実行  
7. MLX adapter から PEFT 互換 artifact への変換  
8. packaging smoke / runbook / promotion rule 出力  
...
## 10. MLX adapter を PEFT submission へ橋渡しする

plan-overview の 6-5 を、v3 で必ず実働させる。  
MLX 学習成果物をそのまま提出しない。

### 10-1. bridge の必須確認

- PEFT 互換 weight file が生成できる
- `adapter_config.json` が正しい
- `base_model_name_or_path` が正しい
- `target_modules` 名が Nemotron 側と一致する
- rank が `32` 以下
...
9. 8 の MLX-to-PEFT bridge  
10. 9 の `shadow_256` / `hard_shadow_256` serious gate  
...
### Step 8. MLX-to-PEFT bridge / packaging

完了条件:

- bridge 検証が通る
- `submission_v3.zip` が出る
- PEFT load smoke が通る

### Step 9. serious gate / runbook / promotion
...
が揃い、top candidate を submit-ready と判断できる
...
uv run python versions/v3/code/train.py train-sft \
  --config versions/v3/conf/train/sft_stage_a_weighted.yaml \
...
uv run python versions/v3/code/train.py convert-mlx-to-peft \
  --adapter-dir versions/v3/outputs/train/p0_stage_a_weighted/adapter \
  --config versions/v3/conf/package/peft_bridge_smoke.yaml \
  --output-dir versions/v3/outputs/packaging/p0_stage_a_weighted
```
...
- MLX training artifact を PEFT 互換へ変換
...
最終的に v3 は、**「基盤を作る版」ではなく、「最初の submit-ready candidate を安定的に作る版」** として完成させる。
...
- MLX-to-PEFT bridge が壊れない
- v1 evaluator で candidate promotion を判断できる
