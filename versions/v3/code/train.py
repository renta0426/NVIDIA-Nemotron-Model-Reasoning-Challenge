...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='v3 Mac-first experimentation and CUDA handoff utilities.')
    subparsers = parser.add_subparsers(dest='command', required=True)

    bootstrap_parser = subparsers.add_parser('bootstrap-v3', help='Create the v3 scaffold and default configs.')
    bootstrap_parser.set_defaults(func=run_bootstrap_v3)
    ...
    cuda_parser.add_argument('--cuda-output-dir', default='/kaggle/working/v3_cuda_candidate')
    cuda_parser.add_argument('--notes', default='')
    cuda_parser.set_defaults(func=run_render_cuda_repro_spec_v3)

    package_peft_parser = subparsers.add_parser(
        'package-peft',
        help='Validate a CUDA-generated PEFT adapter dir and package submission_v3.zip.',
    )
    package_peft_parser.add_argument('--config-path', default=str(DEFAULT_V3_PACKAGE_CONFIG_PATH))
    ...
    runbook_parser = subparsers.add_parser(
        'write-runbook',
        help='Write the v3 candidate registry, promotion rules, and command book.',
    )
    runbook_parser.add_argument('--candidate-registry-path', default=str(DEFAULT_V3_CANDIDATE_REGISTRY_OUTPUT_PATH))
    runbook_parser.add_argument('--promotion-rules-path', default=str(DEFAULT_V3_PROMOTION_RULES_OUTPUT_PATH))
    runbook_parser.add_argument('--output-path', default=str(DEFAULT_V3_TRAINING_RUNBOOK_OUTPUT_PATH))
    runbook_parser.set_defaults(func=run_write_runbook_v3)
