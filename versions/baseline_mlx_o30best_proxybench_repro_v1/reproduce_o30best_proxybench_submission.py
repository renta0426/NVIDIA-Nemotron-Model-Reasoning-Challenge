...
SOURCE_RESUME_RUN_NAME = (
    ...
    "o30best_proxymiss40_text20_grav15_unit15_sym8_lr2e6_len1536_v1"
)
BEST_RUN_NAME = (
    "nemotron_sft_lora_with_cot_v2_mlx_stagefreeze_v2_stage25_attention_qkvo_reanchor_"
    "o30best_proxybench30ao_b10_t10_g15_u15_lr1e6_len1024_from_proxymiss_v1"
)
BEST_DATASET_STEM = "stage25_o30best_proxybench30ao_binary10_text10_grav15_unit15_v1"
FULL_REPRO_RUN_NAME = (
    "nemotron_sft_lora_with_cot_v2_mlx_stagefreeze_v2_stage25_attention_qkvo_reanchor_"
    "o30best_proxybench30ao_b10_t10_g15_u15_lr1e6_len1024_from_proxymiss_repro_v1"
)
...
EXPECTED_LOCAL = {"correct": 235, "rows": 320, "accuracy": 235 / 320}
EXPECTED_PROXY = {"correct": 131, "rows": 200, "accuracy": 131 / 200}
...
def verify_readme_contract(prepare_manifest: dict[str, Any]) -> None:
    ...
    require(
        ...
        "Proxybench frontier does not use rank-32 LoRA as required by README.md",
    )
...
def verify_best_run() -> dict[str, Any]:
    ...
    require(
        dataset_summary.get("sources", {}).get("benchmark_only_proxy_miss_rows") == 30,
        "Proxybench repro expects 30 benchmark-only answer-only rows",
    )
    require(
        dataset_summary.get("sources", {}).get("binary_stabilizer_rows") == 10,
        "Proxybench repro expects 10 binary stabilizer rows",
    )
    require(
        dataset_summary.get("sources", {}).get("text_stabilizer_rows") == 10,
        "Proxybench repro expects 10 text stabilizer rows",
    )
    require(
        dataset_summary.get("sources", {}).get("gravity_stabilizer_rows") == 15,
        "Proxybench repro expects 15 gravity stabilizer rows",
    )
    require(
        dataset_summary.get("sources", {}).get("unit_stabilizer_rows") == 15,
        "Proxybench repro expects 15 unit stabilizer rows",
    )
...
def write_summary(output_root: Path, mode: str, extra: dict[str, Any]) -> Path:
    summary_path = output_root / "o30best_proxybench_repro_summary.json"
    payload = {"mode": mode, **verify_best_run(), **extra}
    dump_json(summary_path, payload)
    markdown_path = output_root / "o30best_proxybench_repro_summary.md"
...
def render_markdown_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# o30best proxybench repro summary",
        "",
        f"- mode: `{payload['mode']}`",
...
def copy_repro_dataset(dataset_csv: Path, dataset_summary_json: Path, dry_run: bool) -> None:
    print(f"$ copy {BEST_DATASET_CSV} -> {dataset_csv}")
    print(f"$ copy {BEST_DATASET_SUMMARY_JSON} -> {dataset_summary_json}")
    if dry_run:
        return
    dataset_csv.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(BEST_DATASET_CSV, dataset_csv)
    summary = load_json(BEST_DATASET_SUMMARY_JSON)
    summary["output_csv"] = str(dataset_csv.relative_to(REPO_ROOT))
    summary["summary_json"] = str(dataset_summary_json.relative_to(REPO_ROOT))
    summary["copied_from"] = {
        "dataset_csv": str(BEST_DATASET_CSV),
        "dataset_summary_json": str(BEST_DATASET_SUMMARY_JSON),
    }
    dump_json(dataset_summary_json, summary)
...
def command_full_reproduce(args: argparse.Namespace) -> int:
    ...
    copy_repro_dataset(dataset_csv, dataset_summary_json, dry_run=args.dry_run)
    run_command(
        uv_command(
    ...
    run_command(
        uv_command(
            ...
            run_root,
            "--label",
            "o30best proxybench local-best repro v1",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Single-file reproduction pipeline for the current o30best proxybench MLX local/proxy best run. "
    ...
    verify = subparsers.add_parser("verify", help="Verify the tracked proxybench frontier and optionally write a summary.")
    verify.add_argument(
        "--output-root",
        type=Path,
        default=REPO_ROOT / "baseline_mlx" / "outputs" / "o30best_proxybench_local_best_repro_verify_v1",
    )
    verify.set_defaults(func=command_verify)

    export_existing = subparsers.add_parser(
        "export-existing",
        help="Re-run audit/export on the completed proxybench frontier and regenerate submission.zip.",
    )
    export_existing.add_argument(
        "--output-root",
        type=Path,
        default=REPO_ROOT / "baseline_mlx" / "outputs" / "o30best_proxybench_local_best_repro_export_v1",
    )
    ...
    full_reproduce = subparsers.add_parser(
        "full-reproduce",
        help="Copy the proxybench dataset, rerun the short train, and postprocess the full suite.",
    )
    ...
    full_reproduce.add_argument(
        "--summary-output-root",
        type=Path,
        default=REPO_ROOT / "baseline_mlx" / "outputs" / "o30best_proxybench_local_best_repro_full_v1",
    )
    ...
    full_reproduce.add_argument(
        "--publish-commit-message",
        default="Record o30best proxybench local-best repro full rerun results",
    )
