...
RESULTS_MD = VERSION_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_tail_only_boost-results.md"
...
DEFAULT_BUNDLE_PATH = TRAINING_SFT_ROOT / "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_tail_only_boost_bundle.jsonl"
...
def build_summary(
    ...
    return {
        "version": "v20_corrective_corpus_v6_promptlocal_short_token_numeric_guess_tail_only_boost",
        "created_at": utc_now(),
...
def build_training_bundle(
    ...
    manifest = {
        ...
        "note": (
            ...
            "a numeric answer-only support lane restricted to low-frequency equation_numeric_guess operator rows with extra repetition on the tail, and a heavier anti-default1 pack, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the v6 promptlocal-short-token-numeric-guess-tail-only-boost corrective corpus for the v20 Kaggle reproduction path, "
            "keeping the v20 base, reallocating more budget into prompt-local exact rows, "
            "adding both short-closure and token-skill lanes, adding a numeric answer-only support lane that keeps only low-frequency equation_numeric_guess rows with extra tail repetition, "
            "and strengthening the anti-default1 lane while retaining only a minimal boxed-surface stabilizer lane."
        )
    )
    parser.add_argument("--run-name", default="v6_promptlocal_short_token_numeric_guess_tail_only_boost_default")
...
def main() -> None:
    ...
    report_lines = [
        "# v20 corrective corpus v6 promptlocal-short-token-numeric-guess-tail-only-boost report",
        "",
        "## Purpose",
        "",
        ...
        "adds a numeric answer-only support lane for equation_numeric rows with tail-only row coverage and extra tail repetition,",
